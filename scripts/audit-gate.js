...
/**
 * Security allowlist for known, currently-unavoidable transitive advisories.
 * Any new vulnerable package not listed here fails CI immediately.
 */
const ALLOWLIST_BY_WORKSPACE = {
  root: new Set(),
  sdk: new Set(),
};
    ...
    for (const finding of findings) {
      ...
      if (workspaceAllowlist.has(finding.packageName)) {
        allowedFindings.push({
          ...finding,
          reason: ALLOWLIST_REASON[finding.packageName] || 'Allowlisted transitive dependency.',
        });
      } else {
        disallowedFindings.push({
          ...finding,
