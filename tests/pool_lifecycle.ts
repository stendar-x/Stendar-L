...
describe("Pool lifecycle", () => {
  ...
  it("covers pool lifecycle and deposit/withdraw guardrails", async () => {
    ...
    let updatedPool = (await program.account.poolState.fetch(pool)) as any;
  ...
  it("updates depositor yield preference with signer and value validation", async () => {
    ...
    let depositAccount = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
    ...
    depositAccount = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
  ...
  it("compounds pool yield for a depositor and updates accounting invariants", async function () {
    ...
    try {
      ...
      const poolBefore = (await program.account.poolState.fetch(pool)) as any;
      const depositBefore = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
      ...
      const poolAfter = (await program.account.poolState.fetch(pool)) as any;
      const depositAfter = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
  ...
  it("enforces bot preference checks and preserves depositor fallback compound", async function () {
    ...
    try {
      ...
      const afterDepositorFallback = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
      assert.isTrue(
      ...
      const afterBotCompound = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
      assert.isTrue(
  ...
  it("splits frontend fees during compound and accrues yield on compounded principal", async function () {
    ...
    try {
      ...
      const firstDepositState = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
      ...
      assert.equal(frontendFeeDelta.toString(), (totalYieldFee / 2n).toString());
      ...
      await (program.methods as any)
        ...
        .signers([depositor])
        .rpc();
      const secondDepositState = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
  ...
  it("bot auto-claims for CLAIM preference and preserves manual claim fallback", async function () {
    ...
    try {
      ...
      const depositStateBeforeBotClaim = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
      await (program.methods as any)
        .botClaimPoolYield()
        .accountsPartial({
      ...
      const depositStateAfterBotClaim = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
      ...
      const finalDepositState = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
      assert.isTrue(
  ...
  it("splits frontend fees during bot claim and tracks total_yield_claimed", async function () {
    ...
    try {
      ...
      const depositBeforeClaim = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
      await (program.methods as any)
        .botClaimPoolYield()
        .accountsPartial({
          botAuthority: provider.wallet.publicKey,
      ...
      const depositAfterClaim = (await program.account.poolDeposit.fetch(poolDeposit)) as any;
  ...
  it("routes pool fee shares to frontends and falls back when frontend accounts are omitted", async function () {
    ...
    const expectedFrontendShareOne = expectedDepositFeeOne / 2n;
    assert.equal(
    ...
    assert.equal(frontendYieldDeltaOne.toString(), (totalYieldFeeOne / 2n).toString());
    ...
    const expectedFrontendShareTwo = expectedDepositFeeTwo / 2n;
    assert.equal(
