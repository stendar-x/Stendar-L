...
import {
  ...
  expectTxFailure,
  extractAnchorErrorMessage as parseErrorMessage,
  hasIdlInstruction,
  isValidSplMint,
...
describe("Pool lifecycle", () => {
  ...
  async function ensureCollateralRegistry(oraclePriceFeed: PublicKey): Promise<void> {
    ...
    const exists = registry.collateralTypes.some((entry: { mint: PublicKey }) =>
  ...
  it("covers pool lifecycle and deposit/withdraw guardrails", async () => {
    ...
    await expectTxFailure(
      program.methods
        .requestPoolWithdrawal(toBn(20_000_000n))
        .accountsPartial({
          depositor: depositor.publicKey,
          pool,
          poolDeposit,
  ...
  it("allows operator to return only free liquidity while paused", async () => {
    ...
    await airdropSol(connection, wrongDestinationOwner, 0.01);
    const wrongDestinationAta = await getOrCreateAssociatedTokenAccount(
      connection,
      payer,
      usdcMint,
    ...
    await expectTxFailure(
      program.methods
        ...
        .accountsPartial({
          ...
          poolDeposit,
          depositor: depositor.publicKey,
          depositorUsdcAta: wrongDestinationAta.address,
          poolVault,
          tokenProgram: TOKEN_PROGRAM_ID,
        })
        .signers([operator])
    ...
    assert.equal(
      (postBalance - preBalance).toString(),
      "30000000",
      "operator return should move exactly the free-liquidity remainder",
    );
  });
  ...
  it("deploys pool funds to contract, handles queue withdrawals, and claims escrow", async () => {
    ...
    const depositorBalanceBeforeProcess = (await getAccount(connection, depositorAta.address)).amount;
    await program.methods
    ...
    assert.equal(
      (depositorTokenAccount.amount - depositorBalanceBeforeProcess).toString(),
      "100000000",
      "queued withdrawal processing should return the exact requested amount",
    );
