...
function assertEnumVariant(enumObj: unknown, expectedKey: string): void {
...
describe("Stream 12 testing audit", () => {
  ...
  async function getCanonicalUsdcMint(): Promise<PublicKey> {
    ...
    if (!treasury.usdcMint.equals(PublicKey.default)) {
  ...
  async function ensureControllableCollateralType(
    ...
    let registry = (await program.account.collateralRegistry.fetch(collateralRegistryPda)) as any;
    ...
    if (!target) {
      ...
      await program.methods
        ...
        .rpc();
      registry = (await program.account.collateralRegistry.fetch(collateralRegistryPda)) as any;
  ...
  async function createContractFixture(params: {
    ...
    await airdropSol(connection, borrower.publicKey, 6);
  ...
  ): Promise<{ botUsdcAta: PublicKey; botCollateralAta: PublicKey }> {
  ...
  it("exposes stream12 support instructions in current IDL", function () {
    const instructionNames = new Set(program.idl.instructions.map((ix) => ix.name));
    ...
    assert.isTrue(instructionNames.has("initializeMockOraclePriceFeed"));
    assert.isTrue(instructionNames.has("setMockOraclePriceFeed"));
  ...
  it("rejects unauthorized treasury initialization attempts", async () => {
    ...
    await airdropSol(connection, attacker.publicKey, 1);
    ...
    try {
      ...
      assert.match(
        ...
        /(InvalidAuthority|AccountNotSigner|already in use|already initialized)/i,
      );
    }
  });

  it("enforces pause gates on add collateral and recall flows", async () => {
  ...
  it("updates program-owned mock oracle accounts deterministically", async () => {
    ...
    const feedAccount = (await program.account.mockOraclePriceFeed.fetch(feed)) as any;
  ...
  it("runs full standard lifecycle with escrow distribution and token claims", async () => {
    const oracleSeed = nextOracleSeed();
    const fixture = await createContractFixture({
      loanType: "committed",
    ...
    assertEnumVariant(contractAfterFunding.status, "active");
    ...
    assert.equal(
      borrowerUsdcAfterFunding.amount.toString(),
    ...
    assertEnumVariant(contractAfter.status, "completed");
    assert.equal(contractAfter.outstandingBalance.toNumber(), 0);
    assert.equal(contractCollateralBalance.amount.toString(), "0");
  });

  it("rejects unapproved collateral and oracle mismatch on contract creation", async () => {
  ...
  it("enforces stale oracle and LTV floor boundaries", async () => {
    ...
    await airdropSol(connection, floorBorrower.publicKey, 5);
  ...
  it("enforces committed-loan floor minimum for collateral type", async () => {
    const oracleSeed = nextOracleSeed();
  ...
  it("supports collateral top-up and rejects invalid top-ups", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
    ...
    const attacker = anchor.web3.Keypair.generate();
    await airdropSol(connection, attacker.publicKey, 2);
    try {
      await program.methods
  ...
  it("allows top-up during pending recall and rejects top-up on v1 contracts", async () => {
    const fixture = await createContractFixture({
      loanType: "demand",
    ...
    await airdropSol(connection, v1Borrower.publicKey, 5);
    ...
    const dummyMint = await createMint(connection, payer, provider.wallet.publicKey, null, 8);
    const dummyBorrowerAta = await getOrCreateAssociatedTokenAccount(
      connection,
      payer,
      dummyMint,
      v1Borrower.publicKey,
    );
    const dummyContractAta = await getOrCreateAssociatedTokenAccount(
      connection,
      payer,
      dummyMint,
      v1ContractPda,
    ...
    try {
      await program.methods
        .addCollateral(new anchor.BN(500))
        .accounts({
          contract: v1ContractPda,
          state: statePda,
          borrower: v1Borrower.publicKey,
          borrowerCollateralAta: dummyBorrowerAta.address,
          contractCollateralAta: dummyContractAta.address,
          tokenProgram: TOKEN_PROGRAM_ID,
        })
        .signers([v1Borrower])
        .rpc();
      assert.fail("expected add_collateral on v1 contract to fail");
    } catch (error) {
      assert.match(parseErrorMessage(error), /(InvalidContractVersion|TokenAccountMismatch)/);
    }
  });

  it("supports wSOL collateral top-up and unwrap on completion", async () => {
  ...
  it("supports full liquidation seizure for wSOL collateral", async () => {
    const oracleFeedSeed = nextOracleSeed();
    const oraclePrice = 250_000_000_000n;
    const oracleFeed = await upsertMockOraclePriceFeed(oracleFeedSeed, oraclePrice, -8);
    const wsolCollateralType = await ensureCollateralTypeForMint(
  ...
  it("enforces partial liquidation health checks and 50% cap", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
    ...
    try {
      ...
      await airdropSol(connection, unauthorized.publicKey, 2);
      await program.methods
    ...
    const expectedCap = (BigInt(before.outstandingBalance.toString()) * 5_000n) / 10_000n;
    assert.equal(
      after.outstandingBalance.toString(),
      (BigInt(before.outstandingBalance.toString()) - expectedCap).toString(),
    );
    assert.equal(escrowAfter.availablePrincipal.toString(), expectedCap.toString());
  });

  it("supports sequential partial liquidations, seizure math, and escrow claims", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
      targetAmount: 1_000_000_000n,
      ltvFloorBps: 11_000,
    ...
    const cappedRepay = (beforeOutstanding * 5_000n) / 10_000n;
    assert.equal(
      afterFirst.outstandingBalance.toString(),
      (beforeOutstanding - cappedRepay).toString(),
      "first partial liquidation should apply 50% outstanding cap",
    );
    assert.equal(
      escrowAfterFirst.availablePrincipal.toString(),
      cappedRepay.toString(),
      "single lender should receive full capped repay in escrow",
    );
  ...
  it("rejects partial liquidation for stale oracle and legacy v1 contracts", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
    ...
    const v1Borrower = anchor.web3.Keypair.generate();
    const v1Lender = anchor.web3.Keypair.generate();
    await Promise.all([
      airdropSol(connection, v1Borrower.publicKey, 5),
      airdropSol(connection, v1Lender.publicKey, 5),
    ]);
    ...
    const [v1ContributionPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("contribution"), v1ContractPda.toBuffer(), v1Lender.publicKey.toBuffer()],
      program.programId,
    );
    const [v1EscrowPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("escrow"), v1ContractPda.toBuffer(), v1Lender.publicKey.toBuffer()],
      program.programId,
    );

    const v1Target = new anchor.BN(40_000_000);
    await program.methods
      .createDebtContract(
        v1Seed,
        v1Target,
    ...
    await program.methods
      .contributeToContract(v1Target)
      .accounts({
        contract: v1ContractPda,
        contribution: v1ContributionPda,
        escrow: v1EscrowPda,
        lender: v1Lender.publicKey,
        borrower: v1Borrower.publicKey,
    ...
    const dummyMint = await createMint(connection, payer, provider.wallet.publicKey, null, 6);
    const botUsdcAtaV1 = await getOrCreateAssociatedTokenAccount(
      connection,
      payer,
      dummyMint,
      provider.wallet.publicKey,
    );
    const contractUsdcAtaV1 = await getOrCreateAssociatedTokenAccount(
      connection,
      payer,
      dummyMint,
      v1ContractPda,
    ...
    const contractCollateralAtaV1 = await getOrCreateAssociatedTokenAccount(
      connection,
      payer,
      dummyMint,
      v1ContractPda,
    ...
    const botCollateralAtaV1 = await getOrCreateAssociatedTokenAccount(
      connection,
      payer,
      dummyMint,
      provider.wallet.publicKey,
    );
    const escrowUsdcAtaV1 = await getOrCreateAssociatedTokenAccount(
      connection,
      payer,
      dummyMint,
      v1EscrowPda,
    ...
    try {
      await program.methods
        .partialLiquidate(new anchor.BN(1))
        .accounts({
          contract: v1ContractPda,
          state: statePda,
          collateralRegistry: collateralRegistryPda,
          priceFeedAccount: fixture.collateralType.oraclePriceFeed,
          botUsdcAta: botUsdcAtaV1.address,
          contractUsdcAta: contractUsdcAtaV1.address,
          contractCollateralAta: contractCollateralAtaV1.address,
          botCollateralAta: botCollateralAtaV1.address,
          treasury: treasuryPda,
          botAuthority: provider.wallet.publicKey,
          borrower: v1Borrower.publicKey,
          tokenProgram: TOKEN_PROGRAM_ID,
        ...
        .remainingAccounts([
          { pubkey: v1ContributionPda, isSigner: false, isWritable: false },
          { pubkey: v1EscrowPda, isSigner: false, isWritable: true },
          { pubkey: escrowUsdcAtaV1.address, isSigner: false, isWritable: true },
        ])
        .rpc();
      assert.fail("expected partial liquidation on v1 contract to fail");
    } catch (error) {
      assert.match(parseErrorMessage(error), /(InvalidContractVersion|TokenAccountMismatch)/);
    }
  });

  it("supports full liquidation on price trigger", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
    ...
    const stateBefore = (await program.account.state.fetch(statePda)) as any;
    ...
    const treasuryAfter = (await program.account.treasury.fetch(treasuryPda)) as any;
    const stateAfter = (await program.account.state.fetch(statePda)) as any;
    assertEnumVariant(contractAfter.status, "liquidated");
    ...
    assert.isTrue(
      ...
      "liquidation should accrue treasury fees",
    );
    assert.equal(
  ...
  it("supports time-triggered full liquidation for committed loans", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
  ...
  it("supports full liquidation when a demand contract enters pending recall", async () => {
    const fixture = await createContractFixture({
      loanType: "demand",
      targetAmount: 350_000_000n,
      ltvFloorBps: 11_000,
  ...
  it("handles bad debt and supports partial-then-full liquidation progression", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
    ...
    assertEnumVariant(contractAfter.status, "liquidated");
    assert.isTrue(
      BigInt(contractAfter.outstandingBalance.toString()) > 0n,
      "bad debt path should leave residual outstanding debt",
    );
    assert.isTrue(
      BigInt(escrowAfter.availablePrincipal.toString()) < fixture.targetAmount,
      "escrow principal should reflect collateral-limited recovery",
    );
  });

  it("supports demand recall request and borrower repay flow", async () => {
    const fixture = await createContractFixture({
      loanType: "demand",
      targetAmount: 1_000_000_000n,
      ltvFloorBps: 11_000,
    ...
    assertEnumVariant(afterRepay.status, "active");
  ...
  it("enforces recall grace period before bot processing", async () => {
    const fixture = await createContractFixture({
      loanType: "demand",
      targetAmount: 1_000_000_000n,
      ltvFloorBps: 11_000,
  ...
  it("processes recall after grace period with fee accounting and escrow payout", async () => {
    const fixture = await createContractFixture({
      loanType: "demand",
      targetAmount: 1_000_000_000n,
      ltvFloorBps: 11_000,
    ...
    const treasuryAfter = (await program.account.treasury.fetch(treasuryPda)) as any;
    assertEnumVariant(contractAfter.status, "active");
  ...
  it("marks contract completed when last lender recall is processed", async () => {
    const fixture = await createContractFixture({
      loanType: "demand",
      targetAmount: 300_000_000n,
      ltvFloorBps: 11_000,
  ...
  it("rejects borrower_repay_recall without pending recall and after grace expiry", async () => {
    const fixture = await createContractFixture({
      loanType: "demand",
      targetAmount: 300_000_000n,
      ltvFloorBps: 11_000,
  ...
  it("verifies value/collateral conservation and rent safety invariants", async () => {
    const oracleSeed = nextOracleSeed();
    const fixture = await createContractFixture({
      loanType: "committed",
      targetAmount: 120_000_000n,
  ...
  it("preserves recall LTV ratio and exact 2% recall fee", async () => {
    const oracleSeed = nextOracleSeed();
    const fixture = await createContractFixture({
      loanType: "demand",
      targetAmount: 100_000_000n,
  ...
  it("rejects process_recall account substitution across lender escrows", async () => {
    const oracleSeed = nextOracleSeed();
    const fixture = await createContractFixture({
      loanType: "demand",
      targetAmount: 100_000_000n,
  ...
  it("handles dust-sized lifecycle amounts without arithmetic faults", async () => {
    const oracleSeed = nextOracleSeed();
    const fixture = await createContractFixture({
      loanType: "committed",
      targetAmount: 1_000n,
  ...
  it("supports cancel with SPL collateral return", async () => {
    const oracleSeed = nextOracleSeed();
    const fixture = await createContractFixture({
      loanType: "committed",
    ...
    assertEnumVariant(contractAfter.status, "cancelled");
    assert.equal(contractAfter.collateralAmount.toString(), "0");
    assert.equal(
  ...
  it("supports cancel with wSOL collateral unwrap", async () => {
    const oracleFeedSeed = nextOracleSeed();
    const oraclePrice = 250_000_000_000n;
    const oracleFeed = await upsertMockOraclePriceFeed(oracleFeedSeed, oraclePrice, -8);
    const wsolCollateralType = await ensureCollateralTypeForMint(
  ...
  it("supports lender USDC refund after cancellation", async () => {
    const oracleSeed = nextOracleSeed();
    const fixture = await createContractFixture({
      loanType: "committed",
      targetAmount: 120_000_000n,
  ...
  it("keeps v1 cancel/refund compatibility with null token accounts", async () => {
    const borrower = anchor.web3.Keypair.generate();
    const lender = anchor.web3.Keypair.generate();
    await Promise.all([
    ...
    const partialContribution = new anchor.BN(40_000_000);
    await program.methods
      .createDebtContract(
        contractSeed,
        targetAmount,
    ...
    const contributionAfter = await program.account.lenderContribution.fetch(contributionPda);
    assertEnumVariant(contractAfter.status, "cancelled");
    assert.equal(contractAfter.fundedAmount.toString(), "0");
    assert.equal(contributionAfter.isRefunded, true);
  });

  it("runs a v1 lifecycle conservation sanity flow", async () => {
    const borrower = anchor.web3.Keypair.generate();
    const lender = anchor.web3.Keypair.generate();
    await Promise.all([
      airdropSol(connection, borrower.publicKey, 5),
    ...
    await program.methods
      .makePaymentWithDistribution(targetAmount)
      .accounts({
        contract: contractPda,
        operationsFund: operationsFundPda,
        state: statePda,
  ...
  it("rejects request_recall on legacy v1 contracts", async () => {
    const borrower = anchor.web3.Keypair.generate();
    const lender = anchor.web3.Keypair.generate();
    await Promise.all([
      airdropSol(connection, borrower.publicKey, 5),
    ...
    try {
      await program.methods
        ...
        .signers([lender])
        .rpc();
      assert.fail("expected request_recall to fail for v1 contracts");
    } catch (error) {
      assert.match(parseErrorMessage(error), /(InvalidContractVersion|AccountNotSigner)/);
    }
  });
});
