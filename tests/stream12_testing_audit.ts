...
describe("Stream 12 testing audit", () => {
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      for (const entry of (registry as any).collateralTypes) {
  ...
  it("rejects unauthorized callers for makePaymentWithDistribution, claimFromEscrow, and cancelContract", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
      targetAmount: 100_000_000n,
      ltvFloorBps: 11_000,
  ...
  it("rejects zero-amount contributions and self-funding contributions", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
  ...
  it("rejects full liquidation when oracle price data is stale", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
      targetAmount: 400_000_000n,
      ltvFloorBps: 11_000,
  ...
  it("supports full liquidation on price trigger, returns excess collateral, and tracks a 3% fee", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
  ...
  it("supports full liquidation when underwater with no borrower excess", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
  ...
  it("supports full liquidation at the collateral equals debt boundary", async () => {
    const fixture = await createContractFixture({
      loanType: "committed",
      targetAmount: 500_000_000n,
      ltvFloorBps: 11_000,
  ...
  it("rejects lender refund while contract is still active", async () => {
    const oracleSeed = nextOracleSeed();
    const fixture = await createContractFixture({
      loanType: "committed",
      targetAmount: 120_000_000n,
  ...
  it("rejects refund_lender when signer is not the contribution lender", async () => {
    const oracleSeed = nextOracleSeed();
    const fixture = await createContractFixture({
      loanType: "committed",
      targetAmount: 120_000_000n,
  ...
  it("prevents double refund of the same contribution", async () => {
    const oracleSeed = nextOracleSeed();
    const fixture = await createContractFixture({
      loanType: "committed",
      targetAmount: 120_000_000n,
