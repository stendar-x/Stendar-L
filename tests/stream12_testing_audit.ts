...
type CollateralRegistryEntry = ActiveCollateralType & { isActive: boolean };
...
describe("Stream 12 testing audit", () => {
  ...
  async function ensureControllableCollateralType(
    ...
    let target = registry.collateralTypes.find(
      (entry: CollateralRegistryEntry) => entry.isActive === true && !(entry.mint as PublicKey).equals(NATIVE_MINT),
    );
    if (!target) {
      target = registry.collateralTypes.find((entry: CollateralRegistryEntry) => entry.isActive === true);
    }
    ...
    if (!target) {
      ...
      target = registry.collateralTypes.find((entry: CollateralRegistryEntry) => entry.mint.equals(mint));
      ...
      await program.methods
        ...
        .rpc();
      registry = await program.account.collateralRegistry.fetch(collateralRegistryPda);
      target = registry.collateralTypes.find((entry: CollateralRegistryEntry) => entry.mint.equals(target.mint));
    }
  ...
  async function ensureCollateralTypeForMint(
    ...
    await ensureCollateralRegistryInitialized();
    let registry = await program.account.collateralRegistry.fetch(collateralRegistryPda);
    let target = registry.collateralTypes.find((entry: CollateralRegistryEntry) => entry.mint.equals(mint));

    if (!target) {
      const hasInactiveEntry = registry.collateralTypes.some((entry: CollateralRegistryEntry) => !entry.isActive);
      if (
  ...
  it("rejects withdraw_contribution while escrow balances remain", async function () {
    if (!mockOracleAvailable) return this.skip();
    const fixture = await createContractFixture({
      loanType: "committed",
  ...
  it("rejects zero-amount contributions and self-funding contributions", async function () {
    ...
    await expectTxFailure(
      contributeToContract(fixture, 0n),
  ...
  it("supports full liquidation at the collateral equals debt boundary", async function () {
    ...
    const collateralValueAtBoundary =
      ...
      (10n ** BigInt(fixture.collateralType.decimals) * 100_000_000n);
    assert.include(
      [fixture.targetAmount, fixture.targetAmount - 1n].map((value) => value.toString()),
      collateralValueAtBoundary.toString(),
      "boundary setup should resolve to exact floor-equivalent debt valuation",
    );
  ...
  it("enforces recall grace period boundary at 259_199 and 259_200 seconds", async function () {
    if (!mockOracleAvailable) return this.skip();

    const setRecallBoundaryOffset = async (
      contractPda: PublicKey,
      secondsAfterRequest: number,
    ): Promise<void> => {
      const contract = await program.account.debtContract.fetch(contractPda);
      const recallRequestedAt = Number(contract.recallRequestedAt.toString());
      const now = await getOnchainUnixTimestamp();
      const targetTimestamp = recallRequestedAt + secondsAfterRequest;
      const offset = targetTimestamp - now;
      assert.isAtLeast(
        offset,
        0,
        `computed negative offset ${offset} for ${secondsAfterRequest}s recall boundary`,
      );
      await setTestClockOffset(offset);
    };

    const createRecalledDemandContract = async () => {
      const fixture = await createContractFixture({
        loanType: "demand",
        targetAmount: 300_000_000n,
        ltvFloorBps: 11_000,
      ...
      await program.methods
        ...
        .signers([lender.lender])
        .rpc();
      return { fixture, lender };
    };

    const beforeBoundaryProcess = await createRecalledDemandContract();
    const beforeBoundaryBotAccounts = await getBotLiquidationAccounts(
      beforeBoundaryProcess.fixture.usdcMint,
      beforeBoundaryProcess.fixture.collateralType.mint,
      beforeBoundaryProcess.fixture.borrowerCollateralAta,
    );
    await mintTo(
      connection,
      payer,
      beforeBoundaryProcess.fixture.usdcMint,
      beforeBoundaryBotAccounts.botUsdcAta,
      provider.wallet.publicKey,
      beforeBoundaryProcess.fixture.targetAmount,
    );

    try {
      await setRecallBoundaryOffset(beforeBoundaryProcess.fixture.contractPda, 259_199);

      await expectTxFailure(
        program.methods
          .processRecall()
          .accounts({
            contract: beforeBoundaryProcess.fixture.contractPda,
            botAuthority: provider.wallet.publicKey,
            treasury: treasuryPda,
            contribution: beforeBoundaryProcess.lender.contributionPda,
            escrow: beforeBoundaryProcess.lender.escrowPda,
            botUsdcAta: beforeBoundaryBotAccounts.botUsdcAta,
            contractUsdcAta: beforeBoundaryProcess.fixture.contractUsdcAta,
            escrowUsdcAta: beforeBoundaryProcess.lender.escrowUsdcAta,
            treasuryUsdcAta: beforeBoundaryProcess.fixture.treasuryUsdcAta,
            contractCollateralAta: beforeBoundaryProcess.fixture.contractCollateralAta,
            botCollateralAta: beforeBoundaryBotAccounts.botCollateralAta,
            borrower: beforeBoundaryProcess.fixture.borrower.publicKey,
            state: statePda,
            testClockOffset: testClockOffsetPda,
            tokenProgram: TOKEN_PROGRAM_ID,
            systemProgram: SystemProgram.programId,
    ...
    const beforeBoundaryRepay = await createRecalledDemandContract();
    await mintTo(
      connection,
      payer,
      beforeBoundaryRepay.fixture.usdcMint,
      beforeBoundaryRepay.fixture.borrowerUsdcAta,
      provider.wallet.publicKey,
      beforeBoundaryRepay.fixture.targetAmount,
    );
    try {
      await setRecallBoundaryOffset(beforeBoundaryRepay.fixture.contractPda, 259_199);
      await program.methods
        .borrowerRepayRecall()
        .accounts({
          contract: beforeBoundaryRepay.fixture.contractPda,
          borrower: beforeBoundaryRepay.fixture.borrower.publicKey,
          contribution: beforeBoundaryRepay.lender.contributionPda,
          escrow: beforeBoundaryRepay.lender.escrowPda,
          borrowerUsdcAta: beforeBoundaryRepay.fixture.borrowerUsdcAta,
          contractUsdcAta: beforeBoundaryRepay.fixture.contractUsdcAta,
          escrowUsdcAta: beforeBoundaryRepay.lender.escrowUsdcAta,
          contractCollateralAta: beforeBoundaryRepay.fixture.contractCollateralAta,
          borrowerCollateralAta: beforeBoundaryRepay.fixture.borrowerCollateralAta,
          state: statePda,
          testClockOffset: testClockOffsetPda,
          tokenProgram: TOKEN_PROGRAM_ID,
          systemProgram: SystemProgram.programId,
        })
        .signers([beforeBoundaryRepay.fixture.borrower])
        .rpc();
    } finally {
      await setTestClockOffset(0);
    }

    const boundary = await createRecalledDemandContract();

    const boundaryBotAccounts = await getBotLiquidationAccounts(
      boundary.fixture.usdcMint,
      boundary.fixture.collateralType.mint,
      boundary.fixture.borrowerCollateralAta,
    );
    await mintTo(
      connection,
      payer,
      boundary.fixture.usdcMint,
      boundaryBotAccounts.botUsdcAta,
      provider.wallet.publicKey,
      boundary.fixture.targetAmount,
    );

    try {
      await setRecallBoundaryOffset(boundary.fixture.contractPda, 259_200);

      await expectTxFailure(
        program.methods
          .borrowerRepayRecall()
          .accounts({
            contract: boundary.fixture.contractPda,
            borrower: boundary.fixture.borrower.publicKey,
            contribution: boundary.lender.contributionPda,
            escrow: boundary.lender.escrowPda,
            borrowerUsdcAta: boundary.fixture.borrowerUsdcAta,
            contractUsdcAta: boundary.fixture.contractUsdcAta,
            escrowUsdcAta: boundary.lender.escrowUsdcAta,
            contractCollateralAta: boundary.fixture.contractCollateralAta,
            borrowerCollateralAta: boundary.fixture.borrowerCollateralAta,
            state: statePda,
            testClockOffset: testClockOffsetPda,
            tokenProgram: TOKEN_PROGRAM_ID,
            systemProgram: SystemProgram.programId,
          })
          .signers([boundary.fixture.borrower])
          .rpc(),
      ...
      await program.methods
        ...
        .accounts({
          ...
          contribution: boundary.lender.contributionPda,
          escrow: boundary.lender.escrowPda,
          botUsdcAta: boundaryBotAccounts.botUsdcAta,
          contractUsdcAta: boundary.fixture.contractUsdcAta,
          escrowUsdcAta: boundary.lender.escrowUsdcAta,
          treasuryUsdcAta: boundary.fixture.treasuryUsdcAta,
          contractCollateralAta: boundary.fixture.contractCollateralAta,
          botCollateralAta: boundaryBotAccounts.botCollateralAta,
          borrower: boundary.fixture.borrower.publicKey,
          state: statePda,
          testClockOffset: testClockOffsetPda,
          tokenProgram: TOKEN_PROGRAM_ID,
  ...
  it("splits listing fees for registered frontends and falls back when missing/invalid", async function () {
    ...
    try {
      ...
      const expectedFrontendShare = (listingFeeWithFrontend * 8n) / 10n;
  ...
  it("splits recall fees for stored frontends and falls back to treasury otherwise", async function () {
    ...
    const expectedFrontendRecallShare = (expectedRecallFeeSplit * 8n) / 10n;
