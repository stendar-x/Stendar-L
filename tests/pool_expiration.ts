...
import {
  ...
  mintTo,
  TOKEN_PROGRAM_ID,
  } from "@solana/spl-token";  import { airdropSol,
...
describe("Pool auto-expiration", () => {
  ...
  async function ensureCollateralSetup(): Promise<void> {
    ...
    const registry = await program.account.collateralRegistry.fetch(registryPda);
  ...
  it("keeps idle_since at zero when utilization drops but deposits remain", async function () {
    if (typeof program.methods.initializeMockOraclePriceFeed !== "function") return this.skip();
    const bot = Keypair.generate();
    await airdropSol(connection, bot, 0.05);
    await ensureTreasury(bot.publicKey);
  ...
  it("allows bot to close idle pool after 30 days and refunds operator rent", async function () {
  ...
  it("rejects bot close before 30 days", async () => {
    ...
    await expectTxFailure(
      program.methods
        ...
        .signers([bot])
        .rpc(),
      /PoolNotIdle|AccountNotInitialized/,
    );
  });
  ...
  it("rejects bot close when deposits remain non-zero", async () => {
    ...
    await expectTxFailure(
      program.methods
        ...
        .signers([bot])
        .rpc(),
      /PoolNotEmpty|AccountNotInitialized/,
    );
  });

  it("rejects bot close when utilization is non-zero", async function () {
    if (typeof program.methods.initializeMockOraclePriceFeed !== "function") return this.skip();
    const bot = Keypair.generate();
    await airdropSol(connection, bot, 0.05);
    await ensureTreasury(bot.publicKey);
    const { pool, poolVault } = await createUtilizedPoolFixture(bot);
    await expectTxFailure(
      program.methods
        .expireIdlePool()
        .accountsPartial({
  ...
  it("rejects non-bot signer", async function () {
    if (!canWarpClock) return this.skip();
    const bot = Keypair.generate();
  ...
  it("closes pending pool change PDA alongside pool on expiration", async function () {
    if (!canWarpClock) return this.skip();
    const bot = Keypair.generate();
    const operator = Keypair.generate();
    await Promise.all([
    ...
    await program.methods
      .proposePoolChanges(1_500, null, null, null, null, null, null)
      .accounts({
        operator: operator.publicKey,
        pool,
        pendingChange,
