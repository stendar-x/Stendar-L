...
describe("Pool auto-expiration", () => {
  ...
  before(async () => {
    ...
    try {
      ...
      const resp = await (connection as any)._rpcRequest("warpSlot", [slot + 2]);
      canWarpClock = resp && !resp.error;
    } catch {
      canWarpClock = false;
    }
  });
  ...
  it("keeps idle_since at zero when utilization drops but deposits remain", async function () {
    if (typeof (program.methods as any).initializeMockOraclePriceFeed !== "function") return this.skip();
    const bot = Keypair.generate();
    await airdropSol(connection, bot, 0.05);
    await ensureTreasury(bot.publicKey);
