...
interface FrontendCreateAccounts {
  operator: PublicKey;
  usdcAta: PublicKey;
}
...
describe("Term proposal consensus", () => {
  ...
  async function localRegisterFrontendOperator(operator: anchor.web3.Keypair): Promise<PublicKey> {
  ...
  async function initGuardInfrastructure(): Promise<void> {
    ...
    try {
      ...
      if (errStr.includes("CollateralRegistryFull") || errStr.includes("DuplicateCollateralMint") || errStr.includes("CollateralAlreadyRegistered")) {
        ...
        const active = await findMintableCollateralEntry(
          connection,
          registry.collateralTypes as CollateralTypeEntry[],
  ...
  async function createActiveSingleLenderContract(
    ...
    maxLenders = 14,
    frontend: FrontendCreateAccounts | null = null,
    ...
    const createDebtContractAccounts: Record<string, PublicKey | null> = {
      ...
      frontendOperator: frontend?.operator ?? null,
      frontendUsdcAta: frontend?.usdcAta ?? null,
    ...
    await program.methods
      ...
      .accountsPartial(createDebtContractAccounts)
      .signers([borrower])
      .rpc();
  ...
  before(async function (this: Mocha.Context) {
    ...
    if (!hasMockOracleInstructions) {
      return this.skip(); // mock oracle instructions not compiled (requires testing feature)
    }
  ...
  it("applies seller and buyer secondary fees on accepted offers", async () => {
    ...
    const tradeEvent = await program.account.tradeEvent.fetch(tradeEventPda);
    const sellerContributionAfter = await program.account.lenderContribution.fetch(
      fixture.contributionPda,
    );
    const buyerContributionAfter = await program.account.lenderContribution.fetch(
      buyerContributionPda,
    );
    ...
    assert.equal(
      sellerContributionAfter.contributionAmount.toString(),
      "0",
      "acceptance should fully transfer the seller contribution when listing full amount",
    );
    assert.equal(
      buyerContributionAfter.contributionAmount.toString(),
      listingAmount.toString(),
      "acceptance should mint buyer contribution with the exact listing amount",
    );

    const listingInfoAfterAccept = await connection.getAccountInfo(listingPda, "confirmed");
    if (listingInfoAfterAccept !== null) {
      assert.isTrue(
        listingInfoAfterAccept.lamports === 0 ||
          !listingInfoAfterAccept.owner.equals(program.programId),
        "accepted listing account should be closed after settlement",
      );
    }
    const offerInfoAfterAccept = await connection.getAccountInfo(offerPda, "confirmed");
    if (offerInfoAfterAccept !== null) {
      assert.isTrue(
        offerInfoAfterAccept.lamports === 0 || !offerInfoAfterAccept.owner.equals(program.programId),
        "accepted offer account should be closed after settlement",
      );
    }
  });

  it("splits accepted-offer protocol fees with a registered frontend", async function () {
    if (!hasRegisterFrontendInstruction) {
      return this.skip(); // register_frontend instruction not compiled
    }
    ...
    const fixture = await createActiveSingleLenderContract(8_190);
    const buyer = anchor.web3.Keypair.generate();
    const frontend = anchor.web3.Keypair.generate();
    await Promise.all([
      airdropSol(connection, buyer, 0.05),
      airdropSol(connection, frontend, 0.02),
    ]);

    const frontendOperator = await localRegisterFrontendOperator(frontend);
    ...
    const buyerBefore = (await getAccount(connection, buyerUsdcAta.address)).amount;
    const treasuryBefore = (await getAccount(connection, treasuryUsdcAta)).amount;
    const frontendBefore = (await getAccount(connection, frontendUsdcAta.address)).amount;
    ...
    const buyerAfter = (await getAccount(connection, buyerUsdcAta.address)).amount;
    const treasuryAfter = (await getAccount(connection, treasuryUsdcAta)).amount;
    const frontendAfter = (await getAccount(connection, frontendUsdcAta.address)).amount;
    const expectedSellerFee = 10n;
    const expectedBuyerFee = 10n;
    const totalFee = expectedSellerFee + expectedBuyerFee;
    const expectedFrontendShare = (totalFee * 8n) / 10n;

    assert.equal(
      (sellerAfter - sellerBefore).toString(),
      (1_000_000n - expectedSellerFee).toString(),
    );
    assert.equal(
      (buyerBefore - buyerAfter).toString(),
      (1_000_000n + expectedBuyerFee).toString(),
    );
    assert.equal(
      (treasuryAfter - treasuryBefore).toString(),
      (totalFee - expectedFrontendShare).toString(),
    );
    assert.equal(
      (frontendAfter - frontendBefore).toString(),
      expectedFrontendShare.toString(),
    );
  });

  it("splits transfer_lender_position fees with frontend and falls back without one", async function () {
    if (!hasRegisterFrontendInstruction) {
      return this.skip(); // register_frontend instruction not compiled
    }
    ...
    const fixtureWithFrontend = await createActiveSingleLenderContract(8_191);
    const buyerWithFrontend = anchor.web3.Keypair.generate();
    await airdropSol(connection, buyerWithFrontend, 0.05);
    const buyerWithFrontendUsdcAta = await getOrCreateAssociatedTokenAccount(
      connection,
      payer,
      guardUsdcMint,
      buyerWithFrontend.publicKey,
    );
    await mintTo(
      connection,
      payer,
      guardUsdcMint,
      buyerWithFrontendUsdcAta.address,
      provider.wallet.publicKey,
      2_000_000n,
    );
    ...
    const salePrice = new anchor.BN(1_000_000);
    const transferNonce = 23;
    const [transferEventPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("transfer"), fixtureWithFrontend.contributionPda.toBuffer(), Buffer.from([transferNonce])],
      program.programId,
    );
    const [sellerEscrowPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("escrow"), fixtureWithFrontend.contractPda.toBuffer(), fixtureWithFrontend.lender.publicKey.toBuffer()],
      program.programId,
    );
    const [buyerContributionPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("contribution"), fixtureWithFrontend.contractPda.toBuffer(), buyerWithFrontend.publicKey.toBuffer()],
      program.programId,
    );
    const [buyerEscrowPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("escrow"), fixtureWithFrontend.contractPda.toBuffer(), buyerWithFrontend.publicKey.toBuffer()],
      program.programId,
    );

    const treasuryBeforeFrontend = (await getAccount(connection, treasuryUsdcAta)).amount;
    const frontendBeforeFrontend = (await getAccount(connection, frontendUsdcAta.address)).amount;
    await program.methods
      .transferLenderPosition(transferAmount, salePrice, transferNonce)
      .accounts({
        contract: fixtureWithFrontend.contractPda,
        transferEvent: transferEventPda,
        contribution: fixtureWithFrontend.contributionPda,
        sellerEscrow: sellerEscrowPda,
        buyerContribution: buyerContributionPda,
        buyerEscrow: buyerEscrowPda,
        treasury: treasuryPda,
        state: fixtureWithFrontend.statePda,
        seller: fixtureWithFrontend.lender.publicKey,
        buyer: buyerWithFrontend.publicKey,
        buyerUsdcAccount: buyerWithFrontendUsdcAta.address,
        sellerUsdcAccount: fixtureWithFrontend.lenderUsdcAta,
        treasuryUsdcAccount: treasuryUsdcAta,
        frontendOperator,
        frontendUsdcAta: frontendUsdcAta.address,
        tokenProgram: TOKEN_PROGRAM_ID,
      ...
      .signers([fixtureWithFrontend.lender, buyerWithFrontend])
      .rpc();
    const treasuryAfterFrontend = (await getAccount(connection, treasuryUsdcAta)).amount;
    const frontendAfterFrontend = (await getAccount(connection, frontendUsdcAta.address)).amount;
    const totalFee = 20n;
    const expectedFrontendShare = (totalFee * 8n) / 10n;
    assert.equal(
      (treasuryAfterFrontend - treasuryBeforeFrontend).toString(),
      (totalFee - expectedFrontendShare).toString(),
    );
    assert.equal(
      (frontendAfterFrontend - frontendBeforeFrontend).toString(),
      expectedFrontendShare.toString(),
    );

    const fixtureWithoutFrontend = await createActiveSingleLenderContract(8_192);
    const buyerWithoutFrontend = anchor.web3.Keypair.generate();
    await airdropSol(connection, buyerWithoutFrontend, 0.05);
    const buyerWithoutFrontendUsdcAta = await getOrCreateAssociatedTokenAccount(
      connection,
      payer,
      guardUsdcMint,
      buyerWithoutFrontend.publicKey,
    );
    await mintTo(
      connection,
      payer,
      guardUsdcMint,
      buyerWithoutFrontendUsdcAta.address,
      provider.wallet.publicKey,
      2_000_000n,
    );
    const transferNonceNoFrontend = 24;
    const [transferEventNoFrontendPda] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("transfer"),
        fixtureWithoutFrontend.contributionPda.toBuffer(),
        Buffer.from([transferNonceNoFrontend]),
      ],
      program.programId,
    );
    const [sellerEscrowNoFrontendPda] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("escrow"),
        fixtureWithoutFrontend.contractPda.toBuffer(),
        fixtureWithoutFrontend.lender.publicKey.toBuffer(),
      ],
      program.programId,
    );
    const [buyerContributionNoFrontendPda] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("contribution"),
        fixtureWithoutFrontend.contractPda.toBuffer(),
        buyerWithoutFrontend.publicKey.toBuffer(),
      ],
      program.programId,
    );
    const [buyerEscrowNoFrontendPda] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("escrow"),
        fixtureWithoutFrontend.contractPda.toBuffer(),
        buyerWithoutFrontend.publicKey.toBuffer(),
    ...
    const treasuryBeforeNoFrontend = (await getAccount(connection, treasuryUsdcAta)).amount;
    const frontendBeforeNoFrontend = (await getAccount(connection, frontendUsdcAta.address)).amount;
    await program.methods
      .transferLenderPosition(transferAmount, salePrice, transferNonceNoFrontend)
      .accounts({
        contract: fixtureWithoutFrontend.contractPda,
        transferEvent: transferEventNoFrontendPda,
        contribution: fixtureWithoutFrontend.contributionPda,
        sellerEscrow: sellerEscrowNoFrontendPda,
        buyerContribution: buyerContributionNoFrontendPda,
        buyerEscrow: buyerEscrowNoFrontendPda,
        treasury: treasuryPda,
        state: fixtureWithoutFrontend.statePda,
        seller: fixtureWithoutFrontend.lender.publicKey,
        buyer: buyerWithoutFrontend.publicKey,
        buyerUsdcAccount: buyerWithoutFrontendUsdcAta.address,
        sellerUsdcAccount: fixtureWithoutFrontend.lenderUsdcAta,
        treasuryUsdcAccount: treasuryUsdcAta,
        tokenProgram: TOKEN_PROGRAM_ID,
        systemProgram: SystemProgram.programId,
      })
      .signers([fixtureWithoutFrontend.lender, buyerWithoutFrontend])
      .rpc();
    const treasuryAfterNoFrontend = (await getAccount(connection, treasuryUsdcAta)).amount;
    const frontendAfterNoFrontend = (await getAccount(connection, frontendUsdcAta.address)).amount;
    assert.equal((treasuryAfterNoFrontend - treasuryBeforeNoFrontend).toString(), totalFee.toString());
    assert.equal((frontendAfterNoFrontend - frontendBeforeNoFrontend).toString(), "0");
  });
  ...
  it("blocks trade listing while proposal is active", async () => {
    ...
    await expectTxFailure(
      program.methods
        ...
        .signers([fixture.lender])
        .rpc(),
      /ProposalAlreadyActive/i,
    );
  });

  it("blocks direct lender transfer while proposal is active and allows it after resolution", async () => {
    const fixture = await createActiveSingleLenderContract(8_130);
    const proposalPda = await createProposal(fixture, 1, fixture.borrower, 42);
    ...
    const transferAmount = new anchor.BN(1_000_000);
    const salePrice = new anchor.BN(1_000_000);
    const transferNonce = 33;
    const [transferEventPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("transfer"), fixture.contributionPda.toBuffer(), Buffer.from([transferNonce])],
      program.programId,
    );
    const [sellerEscrowPda] = PublicKey.findProgramAddressSync(
      [Buffer.from("escrow"), fixture.contractPda.toBuffer(), fixture.lender.publicKey.toBuffer()],
    ...
    const contributionBefore = await program.account.lenderContribution.fetch(fixture.contributionPda);
    await expectTxFailure(
      program.methods
        .transferLenderPosition(transferAmount, salePrice, transferNonce)
        .accounts({
    ...
    const contributionAfterFailure = await program.account.lenderContribution.fetch(fixture.contributionPda);
    assert.equal(
      contributionAfterFailure.contributionAmount.toString(),
      contributionBefore.contributionAmount.toString(),
    );
    ...
    const sellerContributionAfter = await program.account.lenderContribution.fetch(fixture.contributionPda);
    const buyerContributionAfter = await program.account.lenderContribution.fetch(buyerContributionPda);
    assert.equal(sellerContributionAfter.contributionAmount.toString(), "0");
    assert.equal(buyerContributionAfter.contributionAmount.toString(), transferAmount.toString());
  });
  ...
  it("rejects recall pledge on committed loan proposal creation", async () => {
    ...
    await expectTxFailure(
      ...
      /RecallPledgeNotAllowedForCommittedLoans/i,
    );
  });
  ...
  it("enforces proposal recall grace period and then processes pledged recall", async () => {
    ...
    await expectTxFailure(
      program.methods
        ...
        .rpc(),
      /ProposalRecallGraceNotElapsed/i,
    );
  ...
  it("blocks proposal close until pledged recalls are processed", async () => {
    const fixture = await createActiveSingleLenderContract(8_125, "demand");
    const proposalPda = await createProposal(fixture, 1, fixture.borrower, 43, false);
    await voteOnProposal(
      fixture,
      1,
    ...
    const proposalAfterRecall = await program.account.termAmendmentProposal.fetch(proposalPda);
    assert.equal(proposalAfterRecall.recallsProcessed, 1);
    ...
    if (proposalInfoAfterClose !== null) {
      assert.isTrue(
        proposalInfoAfterClose.lamports === 0 ||
          !proposalInfoAfterClose.owner.equals(program.programId),
        "proposal account should be closed after pledged recalls are fully processed",
      );
    }
    assert.isAbove(
      proposerLamportsAfter,
      proposerLamportsBefore,
      "closing should reclaim rent after pledged recalls are processed",
    );
  });

  it("splits proposal recall fees with stored frontend attribution", async function () {
    if (!hasRegisterFrontendInstruction) {
      return this.skip(); // register_frontend instruction not compiled
    }
    ...
    const fixture = await createActiveSingleLenderContract(
      8_123,
      "demand",
      14,
      {
        operator: frontendOperator,
        usdcAta: frontendUsdcAta.address,
      },
    );
    const proposalPda = await createProposal(fixture, 1, fixture.borrower, 43, false);
    await voteOnProposal(
    ...
    const frontendAfter = (await getAccount(connection, frontendUsdcAta.address)).amount;
    assert.equal((escrowAfter - escrowBefore).toString(), "980000");
    assert.equal((treasuryAfter - treasuryBefore).toString(), "10000");
    assert.equal((frontendAfter - frontendBefore).toString(), "10000");
  });

  it("rejects proposal recall when frontend ATA owner mismatches stored frontend", async function () {
    if (!hasRegisterFrontendInstruction) {
      return this.skip(); // register_frontend instruction not compiled
    }
      ...
      await expectTxFailure(
        program.methods
          ...
          .accountsPartial({
            ...
            state: statePda,
            testClockOffset: testClockOffsetPda,
            frontendUsdcAta: fixture.borrowerUsdcAta,
            tokenProgram: TOKEN_PROGRAM_ID,
            systemProgram: SystemProgram.programId,
          })
          .rpc(),
        /FrontendTokenAccountMismatch/i,
      );
    } finally {
      await setTestClockOffset(0);
    }
  ...
  it("sets recall grace start when an expiring pledged proposal is expired", async function (this: Mocha.Context) {
    ...
    try {
      await warpForwardSlots(connection, 2_000_000);
    } catch {
      return this.skip(); // clock warp unavailable on this validator
    }
