...
describe("Collateral registry admin", () => {
  ...
  it("Initializes registry and stores authority", async () => {
    ...
    assert.equal(registry.collateralTypes.length, 0);
  });
  ...
  it("Adds wBTC collateral and increments count", async () => {
    ...
    assert.equal(registry.numCollateralTypes, 1);
    assert.equal(registry.collateralTypes.length, 1);
  ...
  it("Adds multiple collateral types", async () => {
    ...
    const registry = await fetchRegistry();
    assert.equal(registry.numCollateralTypes, 4);
    assert.equal(registry.collateralTypes.length, 4);
  ...
  it("Rejects non-authority add/update/deactivate operations", async () => {
    ...
    try {
      ...
      assert.match(parseAnchorError(error), /UnauthorizedAuthorityUpdate/);
    }
