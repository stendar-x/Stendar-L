...
describe("Revolving lifecycle", () => {
  ...
  async function ensurePlatformInitialized(): Promise<void> {
    ...
    if (!registryInfo) {
      ...
      const hasTesting =
        typeof program.methods.resetCollateralRegistry === "function";
      if (hasTesting) {
        const reg = await program.account.collateralRegistry.fetch(
          collateralRegistryPda,
        );
        const activeCount = (
          reg.collateralTypes as Array<{ isActive?: boolean }>
        ).filter((entry) => entry?.isActive === true).length;
        if (activeCount >= 8) {
          await program.methods
    ...
    const existingType = registry.collateralTypes.find((entry: { mint: PublicKey }) =>
  ...
  async function setPlatformPaused(paused: boolean): Promise<void> {
  ...
  before(async function (this: Mocha.Context) {
    const hasMockOracleTestInstructions =
      ...
      typeof program.methods.initializeTestClockOffset === "function" &&
      typeof program.methods.setTestClockOffset === "function";
  ...
  it("supports revolving draw/repay/close and standby fee distribution", async () => {
    ...
    const drawRemainingAccounts = [
      { pubkey: contributionPda, isSigner: false, isWritable: false },
      { pubkey: escrowPda, isSigner: false, isWritable: false },
    ];

    // P0-REV-PAUSE regression: paused draw must fail, then the same draw path succeeds once unpaused.
    await setPlatformPaused(true);
    try {
      await expectTxFailure(
        program.methods
          .drawFromRevolving(toBn(drawRaw))
          .accountsPartial(drawAccounts)
          .remainingAccounts(drawRemainingAccounts)
          .signers([borrower])
          .rpc(),
        /PlatformPaused/,
      );
    } finally {
      await setPlatformPaused(false);
    }
    ...
    const standbyFeeDelta =
      BigInt(contractAfterStandby.totalStandbyFeesPaid.toString()) -
      BigInt(contractBeforeStandby.totalStandbyFeesPaid.toString());
    const escrowInterestDelta =
      BigInt(escrowAfter.availableInterest.toString()) -
      BigInt(escrowBefore.availableInterest.toString());
    assert.isTrue(standbyFeeDelta > 0n, "standby distribution should accrue a positive fee amount");
    assert.equal(
      escrowInterestDelta.toString(),
      standbyFeeDelta.toString(),
      "standby fee accrual should exactly match escrow interest increase for a single lender",
    );
    assert.equal(contractAfterStandby.accruedStandbyFees.toString(), "0");
    assert.equal(
      contractAfterStandby.totalStandbyFeesPaid.toString(),
      (
        BigInt(contractBeforeStandby.totalStandbyFeesPaid.toString()) + standbyFeeDelta
      ).toString(),
      "standby fee total should advance by the distributed standby fee amount",
    );
