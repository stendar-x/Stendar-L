...
describe("Pool timelock changes", () => {
  ...
  it("rejects propose_pool_changes when signer is not pool operator", async () => {
    ...
    await expectTxFailure(
      program.methods
        .proposePoolChanges(1_333, null, null, null, null, null, null, null)
        .accounts({
  ...
  it("rejects propose_pool_changes when all proposal fields are None", async () => {
    ...
    await expectTxFailure(
      program.methods
        .proposePoolChanges(null, null, null, null, null, null, null, null)
        .accounts({
          operator: operator.publicKey,
          pool,
          pendingChange,
  ...
  it("applies pending changes after timelock and closes pending PDA", async () => {
    const operator = Keypair.generate();
    await airdropSol(connection, operator, 0.2);
    const operatorAuth = await authorizeOperator(operator);
    ...
    const pendingInfo = await connection.getAccountInfo(pendingChange, "confirmed");
    assert.isNull(pendingInfo, "pending change PDA should be closed after apply");
  ...
  it("enforces timelock boundaries (before expiry fails, at expiry succeeds)", async () => {
    const operator = Keypair.generate();
    await airdropSol(connection, operator, 0.2);
    const operatorAuth = await authorizeOperator(operator);
    const { pool } = await createPoolForOperator({ operator, operatorAuth });
  ...
  it("cancels a pending proposal and leaves pool unchanged", async () => {
    ...
    const pendingInfo = await connection.getAccountInfo(pendingChange, "confirmed");
    assert.isNull(pendingInfo, "pending change PDA should be closed after cancel");
  ...
  it("applies partial updates without mutating untouched pool fields", async () => {
    const operator = Keypair.generate();
    await airdropSol(connection, operator, 0.2);
    const operatorAuth = await authorizeOperator(operator);
    const { pool } = await createPoolForOperator({
