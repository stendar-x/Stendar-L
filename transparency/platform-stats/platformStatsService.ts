interface SolanaReader {
  getAllContracts(): Promise<any[]>;
  getAllContributions(): Promise<any[]>;
}
...
const DEFAULT_LOAN_TYPE_KEYS = ['Demand', 'Committed', 'Unknown'] as const;

const DEFAULT_INTEREST_PAYMENT_TYPE_KEYS = ['OutstandingBalance', 'CollateralTransfer', 'Unknown'] as const;
const DEFAULT_PRINCIPAL_PAYMENT_TYPE_KEYS = ['CollateralDeduction', 'NoFixedPayment', 'Unknown'] as const;
const DEFAULT_PAYMENT_FREQUENCY_KEYS = ['Daily', 'Weekly', 'BiWeekly', 'Monthly', 'Unknown'] as const;
...
class PlatformStatsService {
  private solanaService: SolanaReader;

  constructor(solanaService: SolanaReader) {
  ...
  public async calculatePlatformStats(): Promise<PlatformMetrics> {
    try {
      ...
      try {
        ...
        console.warn('Failed to fetch contracts for platform stats:', error);
        contracts = [];
      }
      ...
      try {
        ...
        console.warn('Failed to fetch contributions for platform stats:', error);
        contributions = [];
      }
      ...
      for (const contract of contracts) {
        try {
          if (!this.isStaleOpenListing(contract?.account)) {
            eligibleContracts.push(contract);
          }
        } catch (error) {
          console.warn('Skipping malformed contract for platform stats:', error);
        }
      }
      ...
      console.error('Platform stats calculation failed:', error);
      return {
        uniqueWallets: 0,
        totalVolume: 0,
        activeContracts: 0,
  ...
  private isStaleOpenListing(account: any): boolean {
    ...
    const expiresAt = safeNumber(account?.expires_at);
  ...
  private processContracts(contracts: any[], calculation: PlatformStatsCalculation): void {
    for (const contract of contracts) {
      try {
        ...
        const targetUsdc = safeUsdcAmount(account?.target_amount);
        ...
        const collateralUsdc = safeUsdcAmount(account?.collateral_amount);
        const principalPaidUsdc = atomicToUsdc(account?.total_principal_paid);
        const interestRatePercent = boundedBasisPointsToPercent(account?.interest_rate, 0, Number.POSITIVE_INFINITY);
        const ltvPercentValue = boundedBasisPointsToPercent(account?.ltv_ratio, 0, Number.POSITIVE_INFINITY);
        const termDays = safeBoundedNumber(account?.term_days, 0, Number.POSITIVE_INFINITY);
        ...
        if (isValidParticipantKey(account.borrower)) {
        ...
        if (statusKey === 'Active' || statusKey === 'PendingRecall') {
          ...
          calculation.totalCollateralLockedActiveUSDC += collateralUsdc ?? 0;
        }
        ...
        if (targetUsdc !== null) calculation.loanSizeValuesUSDC.push(targetUsdc);
        ...
        if (collateralUsdc !== null) calculation.collateralValuesUSDC.push(collateralUsdc);

        if (targetUsdc !== null && targetUsdc > 0) {
  ...
  private processContributions(contributions: any[], calculation: PlatformStatsCalculation): void {
    for (const contribution of contributions) {
      try {
        ...
        if (isValidParticipantKey(account.lender)) {
        ...
        console.warn('Skipping malformed contribution for platform stats:', error);
      }
    }
  }
  ...
  public formatPlatformStats(stats: PlatformMetrics): FormattedPlatformMetrics {
    return {
      uniqueWallets: stats.uniqueWallets.toLocaleString('en-US'),
      totalVolume: `${formatUsdc(stats.totalVolume)} USDC`,
      activeContracts: stats.activeContracts.toLocaleString('en-US'),
      outstandingUSDC: `${formatUsdc(stats.outstandingUSDC)} USDC`,
      totalContracts: stats.totalContracts.toLocaleString('en-US'),
...
function isValidParticipantKey(value: unknown): value is string {
  if (typeof value !== 'string') {
    return false;
  }
  const trimmed = value.trim();
  return trimmed.length > 0 && trimmed !== 'PARSING_ERROR';
}
...
// Use for totals where missing protocol fields should contribute zero.
function atomicToUsdc(atomicUnits: unknown): number {
  const asNumber = safeNumber(atomicUnits);
  return asNumber === null || asNumber < 0 ? 0 : asNumber / 1e6;
}
...
function basisPointsToPercent(value: unknown): number | null {
...
function safeNumber(value: unknown): number | null {
  ...
  if (value && typeof value === 'object') {
    const numericObject = value as { toNumber?: () => number; toString?: () => string };
    let toNumber: (() => number) | undefined;
    let toString: (() => string) | undefined;
    try {
      toNumber = numericObject.toNumber;
      toString = numericObject.toString;
    } catch {
      return null;
    }
    if (typeof toNumber === 'function') {
      try {
        const asNumber = toNumber.call(numericObject);
        if (Number.isFinite(asNumber)) return asNumber;
      } catch {
        // BN.js throws when toNumber() would overflow; fall back to decimal string parsing.
      }
    }
    if (typeof toString === 'function') {
      try {
        const asNumber = Number(toString.call(numericObject));
        if (Number.isFinite(asNumber)) return asNumber;
      } catch {
        return null;
      }
...
// Use for distributions where missing fields should not create observations.
