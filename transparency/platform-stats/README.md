...
`platformStatsService.ts` exposes the public computation logic and receives a
caller-supplied `SolanaReader` adapter.

At a high level, the adapter provides methods to read contract and contribution
account data from Solana RPC, while this module handles the deterministic
aggregation into published platform metrics.
