import { HttpClientConfig, StendarApiError } from './types';
import { safeReadEnv } from './utils/env';

const ABSOLUTE_URL_PATTERN = /^[a-z][a-z0-9+\-.]*:\/\//i;
const RETRYABLE_STATUS_CODES = new Set([429, 502, 503, 504]);

export type RequestRetryPolicy = 'safe' | 'idempotent' | 'never';
...
export class StendarApiClient {
  ...
  private shouldRetryMethod(method: string, retryPolicy: RequestRetryPolicy = 'safe'): boolean {
  ...
  private unwrapEnvelope<T>(response: Response, payload: unknown, options?: RequestOptions): T {
    ...
    if (isEnvelope) {
      if (body.success === true) {
        ...
        if (body.data === undefined && !options?.allowEmptyData) {
          throw new StendarApiError({
            code: `HTTP_${response.status}`,
            message: 'Response envelope data is undefined',
            details: body,
            status: response.status,
            requestId: typeof body.meta === 'object' && body.meta && !Array.isArray(body.meta)
              ? String((body.meta as Record<string, unknown>).requestId || '')
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt += 1) {
      ...
      try {
        ...
        if (this.isRetryableStatus(response.status) && attempt < this.maxRetries) {
          if (this.shouldRetryMethod(upperMethod, options?.retryPolicy)) {
            await this.waitBeforeRetry(attempt);
            continue;
          }
        }

        return this.unwrapEnvelope<T>(response, parsedBody, options);
      } catch (error) {
        ...
        if (
          ...
          this.shouldRetryMethod(upperMethod, options?.retryPolicy) &&
