...
export function validateIdlIntegrity(suppliedIdl: Idl, canonicalIdl: Idl): void {
  ...
  for (const section of sections) {
    ...
    for (const [name] of supplied.entries()) {
      if (!canonical.has(name)) {
        throw new Error(
          `IDL integrity check failed: supplied IDL contains unexpected ${section.slice(0, -1)} "${name}"`
        );
      }
    }

    for (const [name] of suppliedLayouts.entries()) {
      if (!canonicalLayouts.has(name)) {
        throw new Error(
          `IDL integrity check failed: supplied IDL contains unexpected ${section.slice(0, -1)} "${name}" layout`
        );
      }
    }
  }
  ...
  for (const [name] of suppliedTypes.entries()) {
    if (!canonicalTypes.has(name)) {
      throw new Error(`IDL integrity check failed: supplied IDL contains unexpected type "${name}"`);
    }
  }
}
...
export function validateTransactionBuildResponse(response: unknown): TransactionBuildResponse {
  ...
  try {
    decodeSerializedTransaction(normalizedUnsignedTransaction);
  } catch (error) {
