...
export function deriveCollateralRegistryPda(programId?: string): [PublicKey, number] {
  const resolvedProgramId = resolveProgramId(programId);
  return PublicKey.findProgramAddressSync([Buffer.from('collateral_registry')], resolvedProgramId);
}

export function derivePoolPda(
  operatorAddress: string | PublicKey,
  poolSeed: string | number | bigint,
  programId?: string
): [PublicKey, number] {
  const resolvedProgramId = resolveProgramId(programId);
...
export function deriveOperationsFundPda(
  contractAddress: string | PublicKey,
  programId?: string
): [PublicKey, number] {
  const resolvedProgramId = resolveProgramId(programId);
  const contract = toPublicKey(contractAddress);
  return PublicKey.findProgramAddressSync(
...
export function derivePoolDepositPda(
  poolAddress: string | PublicKey,
  depositorAddress: string | PublicKey,
  programId?: string
): [PublicKey, number] {
  const resolvedProgramId = resolveProgramId(programId);
  const pool = toPublicKey(poolAddress);
  const depositor = toPublicKey(depositorAddress);
  return PublicKey.findProgramAddressSync(
...
export function deriveTransferEventPda(
  contributionAddress: string | PublicKey,
  nonce: number,
  programId?: string
): [PublicKey, number] {
  ...
  const contribution = toPublicKey(contributionAddress);
  return PublicKey.findProgramAddressSync(
    [Buffer.from('transfer'), contribution.toBuffer(), toNonceBuffer(nonce)],
    resolvedProgramId
  );
}
