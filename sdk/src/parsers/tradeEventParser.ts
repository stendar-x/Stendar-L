...
import type { ParseAccountOptions, ParsedTradeEventAccount, ParsedTradeType } from './types';

const TRADE_EVENT_RESERVED_BYTES = 96;

export function parseTradeEventAccount(data: Buffer, options?: ParseAccountOptions): ParsedTradeEventAccount | null {
  ...
  try {
    ...
    if (reserved.length !== TRADE_EVENT_RESERVED_BYTES) {
      throw new RangeError('TradeEvent reserved bytes are truncated');
    }
    ...
    return handleParserError('TradeEvent', error, options);
  }
}
