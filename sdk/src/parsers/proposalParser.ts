...
import { handleParserError, mapEnumValue, readOptionU8 } from './parserHelpers';
import type {
  ParseAccountOptions,
...
export function parseProposalAccount(data: Buffer, options?: ParseAccountOptions): ParsedProposalAccount | null {
  ...
  try {
    ...
    if (participantLength > MAX_REASONABLE_PARTICIPANTS) {
      throw new RangeError(`Unreasonable proposal participant count: ${participantLength}`);
    }
    ...
    if (reserved.length !== PROPOSAL_RESERVED_BYTES) {
      throw new RangeError('Proposal reserved bytes are truncated');
    }
    ...
    return handleParserError('TermAmendmentProposal', error, options);
  }
}
