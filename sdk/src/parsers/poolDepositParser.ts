...
import type { ParseAccountOptions, ParsedPoolDepositAccount } from './types';

const POOL_DEPOSIT_RESERVED_BYTES = 88;

export function parsePoolDepositAccount(data: Buffer, options?: ParseAccountOptions): ParsedPoolDepositAccount | null {
  ...
  try {
    ...
    return {
      ...
      deferredYield: asUiUsdc(deferredYieldRaw),
      deferredYieldRaw: deferredYieldRaw.toString(),
      accountVersion,
    };
  } catch (error) {
    return handleParserError('PoolDeposit', error, options);
  }
}
