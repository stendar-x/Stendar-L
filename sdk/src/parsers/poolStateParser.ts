...
import type { ParseAccountOptions, ParsedPoolStateAccount, ParsedPoolStatus } from './types';

const POOL_RESERVED_SIZE = 96;

function decodePoolName(buffer: Buffer, offset: number): string {
  if (!assertMinLength(buffer, offset + 32)) {
    throw new RangeError(`Pool name read out of bounds (offset=${offset}, bufferLength=${buffer.length})`);
  }
...
export function parsePoolStateAccount(data: Buffer, options?: ParseAccountOptions): ParsedPoolStateAccount | null {
  ...
  try {
    ...
    if (reserved.length !== POOL_RESERVED_SIZE) {
      throw new RangeError('PoolState reserved bytes are truncated');
    }
    ...
    return handleParserError('PoolState', error, options);
  }
}
