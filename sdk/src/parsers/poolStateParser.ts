...
const POOL_RESERVED_SIZE = 39;
const TOTAL_PENDING_YIELD_OFFSET = 0;
const PENDING_WITHDRAWAL_REQUESTS_OFFSET = 8;
...
export function parsePoolStateAccount(data: Buffer): ParsedPoolStateAccount | null {
  ...
  try {
    ...
    const reservedStart = offset;
    const totalPendingYieldRaw = readU64(data, reservedStart + TOTAL_PENDING_YIELD_OFFSET);
    const pendingWithdrawalRequests = readU32(
      data,
      reservedStart + PENDING_WITHDRAWAL_REQUESTS_OFFSET,
    );
