...
import type { ParseAccountOptions, ParsedEscrowAccount } from './types';

const LENDER_ESCROW_RESERVED_BYTES = 96;

export function parseEscrowAccount(data: Buffer, options?: ParseAccountOptions): ParsedEscrowAccount | null {
  ...
  try {
    ...
    if (reserved.length !== LENDER_ESCROW_RESERVED_BYTES) {
      throw new RangeError('LenderEscrow reserved bytes are truncated');
    }
    ...
    return handleParserError('LenderEscrow', error, options);
  }
}
