...
import type { ParseAccountOptions } from './types';
...
export function handleParserError(
  parser: string,
  error: unknown,
  options?: ParseAccountOptions
): null {
  options?.onError?.(error, { parser });
  if (options?.strict) {
    if (error instanceof Error) {
      throw error;
    }
    throw new Error(`Failed to parse ${parser}: ${String(error)}`);
  }
  return null;
}
