...
import { handleParserError, mapEnumValue } from './parserHelpers';
import type { ParseAccountOptions, ParsedProposalVoteAccount, ParsedVoteChoice } from './types';

const PROPOSAL_VOTE_RESERVED_BYTES = 96;

export function parseProposalVoteAccount(data: Buffer, options?: ParseAccountOptions): ParsedProposalVoteAccount | null {
  ...
  try {
    ...
    if (reserved.length !== PROPOSAL_VOTE_RESERVED_BYTES) {
      throw new RangeError('ProposalVote reserved bytes are truncated');
    }
    ...
    return handleParserError('ProposalVote', error, options);
  }
}
