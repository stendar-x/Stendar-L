...
import type { ParseAccountOptions, ParsedStateAccount } from './types';

const STATE_RESERVED_BYTES = 96;

export function parseStateAccount(data: Buffer, options?: ParseAccountOptions): ParsedStateAccount | null {
  ...
  try {
    ...
    if (reserved.length !== STATE_RESERVED_BYTES) {
      throw new RangeError('State reserved bytes are truncated');
    }
    ...
    return handleParserError('State', error, options);
  }
}
