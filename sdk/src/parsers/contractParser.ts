...
import { handleParserError, asUiUsdc, mapEnumValue } from './parserHelpers';
import type {
  ParseAccountOptions,
...
const MIN_LAYOUT_LEN = 597;
const RESERVED_TAIL_BYTES = 96;
const MAX_REASONABLE_CONTRIBUTIONS = 100;
...
function readOptionFrequency(data: Buffer, offset: number): { value: ParsedPaymentFrequency | null | 'Unknown'; next: number } {
  ...
  next += 1;
  return {
    value: mapEnumValue<ParsedPaymentFrequency>(FREQUENCY_MAP, frequencyVariant),
    next,
  };
}
...
function parseCurrentLayout(data: Buffer): ParsedContractAccount {
  ...
  const loanType = mapEnumValue<ParsedLoanType>(LOAN_TYPE_MAP, readU8(data, offset));
  offset += 1;
  ...
  const interestPaymentType = mapEnumValue<ParsedInterestPaymentType>(INTEREST_PAYMENT_TYPE_MAP, readU8(data, offset));
  offset += 1;

  const principalPaymentType = mapEnumValue<ParsedPrincipalPaymentType>(PRINCIPAL_PAYMENT_TYPE_MAP, readU8(data, offset));
  offset += 1;

  const interestFrequency = mapEnumValue<ParsedPaymentFrequency>(FREQUENCY_MAP, readU8(data, offset));
  offset += 1;
  ...
  const status = mapEnumValue<ParsedContractStatus>(CONTRACT_STATUS_MAP, readU8(data, offset));
  offset += 1;
  ...
  offset += 8;
  const effectiveDrawableRaw = isRevolving ? availableAmountRaw : 0n;
  ...
  return {
    ...
    effectiveDrawable: asUiUsdc(effectiveDrawableRaw),
    effectiveDrawableRaw: effectiveDrawableRaw.toString(),
    standbyFeeRate,
...
export function parseContractAccount(data: Buffer, options?: ParseAccountOptions): ParsedContractAccount | null {
  ...
  try {
    ...
    return handleParserError('DebtContract', error, options);
  }
}
