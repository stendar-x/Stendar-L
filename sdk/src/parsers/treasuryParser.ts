...
import type { ParseAccountOptions, ParsedTreasuryAccount } from './types';

const TREASURY_RESERVED_BYTES = 96;

export function parseTreasuryAccount(data: Buffer, options?: ParseAccountOptions): ParsedTreasuryAccount | null {
  ...
  try {
    ...
    if (reserved.length !== TREASURY_RESERVED_BYTES) {
      throw new RangeError('Treasury reserved bytes are truncated');
    }
    ...
    return handleParserError('Treasury', error, options);
  }
}
