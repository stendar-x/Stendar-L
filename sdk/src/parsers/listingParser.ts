...
import type { ParseAccountOptions, ParsedListingAccount, ParsedListingType } from './types';

const LISTING_RESERVED_BYTES = 96;

export function parseListingAccount(data: Buffer, options?: ParseAccountOptions): ParsedListingAccount | null {
  ...
  try {
    ...
    if (reserved.length !== LISTING_RESERVED_BYTES) {
      throw new RangeError('TradeListing reserved bytes are truncated');
    }
    ...
    return handleParserError('TradeListing', error, options);
  }
}
