export interface ParserDiagnosticContext {
  parser: string;
}

export interface ParseAccountOptions {
  strict?: boolean;
  onError?: (error: unknown, context: ParserDiagnosticContext) => void;
}
...
export type ParsedPoolStatus = 'Active' | 'Paused' | 'Closed';
...
export interface ParsedContractAccount {
  ...
  effectiveDrawable: number;
  effectiveDrawableRaw: string;
...
export interface ParsedPoolStateAccount {
  operator: string;
  poolSeed: string;
  name: string;
  rateBps: number;
  ...
  currentTotalDepositsRaw: string;
  currentUtilized: number;
  currentUtilizedRaw: string;
  totalYieldDistributed: number;
  totalYieldDistributedRaw: string;
  status: ParsedPoolStatus | 'Unknown';
  createdAt: string;
  authorized: boolean;
  withdrawalQueueEnabled: boolean;
  minimumDeposit: number;
  minimumDepositRaw: string;
  ...
  prevRateBps: number;
  idleSince: string;
  totalPendingYieldRaw: string;
  pendingWithdrawalRequests: number;
  accountVersion: number;
}

export interface ParsedStateAccount {
  authority: string;
  totalDebt: number;
  totalDebtRaw: string;
  ...
  totalPartialLiquidations: string;
  totalContracts: string;
  platformFeeBasisPoints: number;
  poolDepositFeeBps: number;
  poolYieldFeeBps: number;
  primaryListingFeeBps: number;
  secondaryListingFeeBps: number;
  secondaryBuyerFeeBps: number;
  isPaused: boolean;
  accountVersion: number;
}

export interface ParsedTreasuryAccount {
  authority: string;
  pendingAuthority: string;
  botAuthority: string;
  feesCollected: number;
  feesCollectedRaw: string;
  transactionCosts: number;
  transactionCostsRaw: string;
  automatedOperations: string;
  totalContractsProcessed: string;
  lastUpdate: string;
  createdAt: string;
  usdcMint: string;
  treasuryUsdcAccount: string;
  totalLiquidationFees: number;
  totalLiquidationFeesRaw: string;
  totalRecallFees: number;
  totalRecallFeesRaw: string;
  accountVersion: number;
}

export interface ParsedPoolDepositAccount {
  depositor: string;
  pool: string;
  depositAmount: number;
  depositAmountRaw: string;
  accruedYield: number;
  accruedYieldRaw: string;
  lastYieldUpdate: string;
  depositTimestamp: string;
  withdrawalRequested: boolean;
  withdrawalRequestedAt: string;
  withdrawalRequestedAmount: number;
  withdrawalRequestedAmountRaw: string;
  totalYieldClaimed: number;
  totalYieldClaimedRaw: string;
  frontend: string;
  yieldPreference: number;
  totalYieldCompounded: number;
  totalYieldCompoundedRaw: string;
  deferredYield: number;
  deferredYieldRaw: string;
  accountVersion: number;
}
