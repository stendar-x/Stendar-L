...
import {
  BotCloseMaturedRevolvingRequest,
  ...
  RequestRecallRequest,
  RefundLenderRequest,
  StendarClientMode,
  SweepContractPoolRequest,
...
export class LendingActions {
  ...
  requestRecallTransaction(request: RequestRecallRequest): Promise<TransactionBuildResponse> {
  ...
  repayRecallTransaction(request: RepayRecallRequest): Promise<TransactionBuildResponse> {
    const contractAddress = validateSolanaAddress(request.contractAddress, 'contractAddress');
    return this.postTransactionBuild(
  ...
  async sweepContractPoolTransaction(
    request: SweepContractPoolRequest | DirectSweepContractPoolInstructionRequest
  ): Promise<TransactionBuildResponse | TransactionInstruction> {
    if (this.mode === 'direct') {
      return this.requireProgram().sweepContractPool(request);
    }
    const contractAddress = validateSolanaAddress(request.contractAddress, 'contractAddress');
    return this.postTransactionBuild(`/api/contracts/${contractAddress}/sweep-pool`, request);
  }

  async botCloseMaturedRevolvingTransaction(
    request: BotCloseMaturedRevolvingRequest | DirectBotCloseMaturedRevolvingInstructionRequest
  ): Promise<TransactionBuildResponse | TransactionInstruction> {
    if (this.mode === 'direct') {
      return this.requireProgram().botCloseMaturedRevolving(request);
    }
    const contractAddress = validateSolanaAddress(request.contractAddress, 'contractAddress');
    return this.postTransactionBuild(
      `/api/contracts/${contractAddress}/bot-close-matured-revolving`,
      request
    );
  }
