...
import { useScopedEnv } from './env-test-utils';
...
function createMockApiClient() {
  const calls: Array<{ method: 'get' | 'post'; path: string; body?: unknown; options?: unknown }> = [];
  ...
  const api = {
    ...
    post: async <T>(path: string, body?: unknown, options?: unknown): Promise<T> => {
...
test('platform queries include state, stats, and notifications endpoints', async () => {
  ...
  await queries.getPaymentEvents(5000);
  ...
  assert.deepEqual(
    ...
    [
      '/api/platform/state',
      '/api/platform/stats',
      '/api/system/payments/events?limit=1000',
      `/api/system/notifications/${walletAddress}`,
    ]
  );
});

test('platform and community queries reject invalid limit values', () => {
  const { api, calls } = createMockApiClient();
  const platformQueries = new PlatformQueries(api);
  const communityQueries = new CommunityQueries(api);

  assert.throws(
    () => platformQueries.getPaymentEvents(Number.NaN),
    /Invalid limit: expected a positive finite number/i
  );
  assert.throws(
    () => communityQueries.getFeatureLeaderboard(0),
    /Invalid limit: expected a positive finite number/i
  );
...
test('collateral, wallet, and community queries map endpoints', async () => {
  ...
  await community.getFeatureLeaderboard(5000);
  ...
  assert.deepEqual(
    ...
    [
      ...
      '/api/community/feature-requests/leaderboard?limit=1000',
    ]
  );
});
...
test('community admin feature request methods map endpoints and payloads', async () => {
  const { api, calls } = createMockApiClient();
  const actions = new CommunityActions(api);

  await actions.listAdminFeatureRequests('pending');
  await actions.moderateFeatureRequest(42, 'approved');
  await actions.moderateFeatureRequest(43, 'denied', 'Missing implementation details');

  assert.deepEqual(calls, [
    {
      method: 'post',
      path: '/api/community/admin/feature-requests/list',
      body: { status: 'pending' },
    },
    {
      method: 'post',
      path: '/api/community/admin/feature-requests/42/moderate',
      body: { status: 'approved' },
    },
    {
      method: 'post',
      path: '/api/community/admin/feature-requests/43/moderate',
      body: { status: 'denied', moderationNotes: 'Missing implementation details' },
    },
  ]);
});

test('community moderateFeatureRequest validates featureRequestId', () => {
  const { api } = createMockApiClient();
  const actions = new CommunityActions(api);

  assert.throws(() => {
    actions.moderateFeatureRequest(0, 'approved');
  }, /Invalid featureRequestId/);
  assert.throws(() => {
    actions.moderateFeatureRequest(-1, 'approved');
  }, /Invalid featureRequestId/);
  assert.throws(() => {
    actions.moderateFeatureRequest(Number.NaN, 'approved');
  }, /Invalid featureRequestId/);
  assert.throws(() => {
    actions.moderateFeatureRequest(1.5, 'approved');
  }, /Invalid featureRequestId/);
  assert.throws(() => {
    actions.moderateFeatureRequest(Infinity, 'approved');
  }, /Invalid featureRequestId/);
});

test('community applyPoolOperator posts application payload with required fields only', async () => {
  const { api, calls } = createMockApiClient();
  const actions = new CommunityActions(api);

  await actions.applyPoolOperator({
    name: 'Operator Name',
    email: 'operator@example.com',
    experience: '5 years in credit underwriting',
    strategy: 'Conservative fixed-yield strategy',
  });

  assert.deepEqual(calls, [
    {
      method: 'post',
      path: '/api/community/pool-operator-applications',
      body: {
        name: 'Operator Name',
        email: 'operator@example.com',
        experience: '5 years in credit underwriting',
  ...
  assert.equal('walletAddress' in sentBody, false);
  assert.equal('additionalInfo' in sentBody, false);
});

test('community applyPoolOperator includes optional fields when provided', async () => {
  const { api, calls } = createMockApiClient();
  const actions = new CommunityActions(api);
  ...
  await actions.applyPoolOperator({
    ...
    experience: '5 years in credit underwriting',
    strategy: 'Conservative fixed-yield strategy',
    walletAddress: makeAddress(44),
    additionalInfo: 'Can provide monthly portfolio reporting.',
  });
...
test('transaction submission actions opt into idempotent retry policy', async () => {
  const { api, calls } = createMockApiClient();
  const lending = new LendingActions(api, 'api');
  const trading = new TradingActions(api);
  const signedTransactionBase64 = Buffer.from('signed-transaction').toString('base64');
  const expectedWalletAddress = makeAddress(70);
  const expectedAddress = makeAddress(71);
  const contractAddress = makeAddress(72);

  await lending.submitTransaction({ signedTransactionBase64, expectedWalletAddress, contractAddress });
  await trading.submitListingTransaction({ signedTransactionBase64, expectedWalletAddress, expectedAddress });
  await trading.submitOfferTransaction({ signedTransactionBase64, expectedWalletAddress, expectedAddress });

  assert.deepEqual(
    calls.map((call) => ({ path: call.path, options: call.options })),
    [
      {
        path: '/api/contracts/submit-transaction',
        options: { retryPolicy: 'idempotent' },
      },
      {
        path: '/api/trading/listings/submit-transaction',
        options: { retryPolicy: 'idempotent' },
      },
      {
        path: '/api/trading/offers/submit-transaction',
        options: { retryPolicy: 'idempotent' },
      },
    ]
  );
  for (const call of calls) {
    assert.equal(typeof (call.body as { submissionId?: unknown }).submissionId, 'string');
  }
});
