...
test('generateSubmissionIdFromSignedTransaction rejects malformed base64', () => {
  assert.throws(
    () => generateSubmissionIdFromSignedTransaction('not-base64'),
    /Invalid signedTransactionBase64/
  );
  assert.throws(
    () => generateSubmissionIdFromSignedTransaction('A'),
    /Invalid signedTransactionBase64/
  );
});
