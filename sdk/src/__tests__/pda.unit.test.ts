...
test('deriveOperationsFundPda matches on-chain operations_fund seed', () => {
  const programId = new PublicKey(PROGRAM_ID);
  const contract = new PublicKey(makeAddress(314));
  ...
  const [actualPda] = deriveOperationsFundPda(contract, PROGRAM_ID);
  assert.equal(actualPda.toBase58(), expectedPda.toBase58());
});
...
test('derivePoolPda matches on-chain pool seed tuple', () => {
  const programId = new PublicKey(PROGRAM_ID);
  const operator = new PublicKey(makeAddress(777));
  const poolSeed = 12n;

  const [expectedPda] = PublicKey.findProgramAddressSync(
    [Buffer.from('pool'), operator.toBuffer(), toU64LeBuffer(poolSeed)],
    programId
  );

  const [actualPda] = derivePoolPda(operator, poolSeed, PROGRAM_ID);
  assert.equal(actualPda.toBase58(), expectedPda.toBase58());
});
...
test('derivePoolDepositPda matches on-chain pool_deposit seed tuple', () => {
  const programId = new PublicKey(PROGRAM_ID);
  const pool = new PublicKey(makeAddress(555));
  const depositor = new PublicKey(makeAddress(556));
  ...
  const [actualPda] = derivePoolDepositPda(pool, depositor, PROGRAM_ID);
  assert.equal(actualPda.toBase58(), expectedPda.toBase58());
});
...
test('deriveCollateralRegistryPda matches on-chain collateral_registry seed', () => {
  const programId = new PublicKey(PROGRAM_ID);
  const [expectedPda] = PublicKey.findProgramAddressSync([Buffer.from('collateral_registry')], programId);
  const [actualPda] = deriveCollateralRegistryPda(PROGRAM_ID);
  assert.equal(actualPda.toBase58(), expectedPda.toBase58());
});

test('deriveTransferEventPda matches on-chain transfer event seeds', () => {
  const programId = new PublicKey(PROGRAM_ID);
  const contribution = new PublicKey(makeAddress(808));
  const nonce = 9;

  const [expectedPda] = PublicKey.findProgramAddressSync(
    [Buffer.from('transfer'), contribution.toBuffer(), Buffer.from([nonce])],
    programId
  );

  const [actualPda] = deriveTransferEventPda(contribution, nonce, PROGRAM_ID);
  assert.equal(actualPda.toBase58(), expectedPda.toBase58());
});
