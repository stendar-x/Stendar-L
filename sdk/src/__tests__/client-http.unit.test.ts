...
import { withEnv } from './env-test-utils';
...
test('StendarApiClient does not retry on client-initiated timeout (AbortError)', async () => {
  let attempts = 0;
  const client = new StendarApiClient({
    apiUrl: 'https://api.stendar.xyz',
    maxRetries: 2,
  ...
  assert.equal(attempts, 1, 'Timeout should not trigger retries');
});

test('StendarApiClient does not retry POST on network-level errors by default', async () => {
  let attempts = 0;
  const client = new StendarApiClient({
    apiUrl: 'https://api.stendar.xyz',
    maxRetries: 2,
  ...
  assert.equal(attempts, 1, 'POST should not retry ambiguous network errors by default');
});

test('StendarApiClient retries explicitly idempotent POST network errors', async () => {
  let attempts = 0;
  const client = new StendarApiClient({
    apiUrl: 'https://api.stendar.xyz',
    maxRetries: 2,
  ...
  assert.equal(result.ok, true);
  assert.equal(attempts, 2, 'Explicitly idempotent POST should retry network errors');
});
...
test('StendarApiClient honors retryPolicy never for retryable GET responses', async () => {
  let attempts = 0;
  const client = new StendarApiClient({
    apiUrl: 'https://api.stendar.xyz',
    maxRetries: 2,
  ...
  await assert.rejects(
    () => client.get('/api/data', { retryPolicy: 'never' }),
    (error: unknown) => matchesApiError(error, {
      status: 503,
      code: 'HTTP_503',
      message: 'Unavailable',
...
test('StendarApiClient respects maxRetries=0 for both GET and POST', async () => {
  for (const method of ['GET', 'POST'] as const) {
    ...
    assert.equal(attempts, 1, `${method} with maxRetries=0 should not retry`);
  }
});
...
test('StendarApiClient rejects success envelopes without data by default', async () => {
  const client = new StendarApiClient({
    apiUrl: 'https://api.stendar.xyz',
  ...
  await assert.rejects(
    () => client.get<unknown>('/api/missing-data'),
    (error: unknown) => matchesApiError(error, {
      status: 200,
      code: 'HTTP_200',
...
test('StendarApiClient allows empty success envelopes when requested', async () => {
  const client = new StendarApiClient({
    apiUrl: 'https://api.stendar.xyz',
    fetch: (async () => jsonResponse({ success: true })) as typeof globalThis.fetch,
  });

  const response = await client.get<void>('/api/no-content', { allowEmptyData: true });
