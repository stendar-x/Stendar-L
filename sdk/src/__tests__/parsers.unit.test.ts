...
function buildPoolDepositBuffer(): Buffer {
  ...
  w.writeU64(9_000n); // deferred_yield
  w.writeBytes(Buffer.alloc(88));
  w.writeU16(1); // account_version
  return w.toBuffer(268);
}

function buildPoolStateBuffer(): Buffer {
  const w = new BufferWriter();
  w.writeBytes(DISCRIMINATORS.POOL_STATE);
  w.writePubkey(makePubkey(181)); // operator
  w.writeU64(8n); // pool_seed
  w.writeBytes(Buffer.from('PrimePool'.padEnd(32, '\0'), 'utf8')); // name[32]
  w.writeU32(650); // rate_bps
  w.writeU64(20_000_000n); // capacity
  w.writeU64(8_000_000n); // current_total_deposits
  w.writeU64(4_000_000n); // current_utilized
  w.writeU64(250_000n); // total_yield_distributed
  w.writeU8(0); // status: Active
  w.writeI64(1_700_700_000n); // created_at
  w.writeBool(true); // authorized
  w.writeBool(true); // withdrawal_queue_enabled
  w.writeU64(250_000n); // minimum_deposit
  w.writeU32(12); // num_depositors
  w.writePubkey(makePubkey(182)); // loan_mint
  w.writePubkey(makePubkey(183)); // vault_token_account
  w.writeU8(254); // bump
  w.writeU8(1); // allowed_loan_type
  w.writeU16(10_500); // min_ltv_bps
  w.writeU32(365); // max_term_days
  w.writeI64(1_700_705_000n); // rate_updated_at
  w.writeU32(625); // prev_rate_bps
  w.writeI64(1_700_706_000n); // idle_since
  w.writeU64(50_000n); // total_pending_yield
  w.writeU32(3); // pending_withdrawal_requests
  w.writeBytes(Buffer.alloc(96));
  w.writeU16(1); // account_version
  return w.toBuffer();
}
...
test('parseCollateralRegistryAccount returns null when numCollateralTypes does not match vec length', () => {
  const malformed = Buffer.from(buildCollateralRegistryBuffer());
  malformed[40] = 1; // numCollateralTypes
  assert.equal(parseCollateralRegistryAccount(malformed), null);
});
...
test('state, treasury, and pool deposit parsers decode promoted layouts', () => {
  ...
  assert.equal(poolDeposit?.deferredYieldRaw, '9000');
});

test('parsePoolDepositAccount rejects truncated deferred-yield layout buffers', () => {
  const truncatedBeforeDeferred = buildPoolDepositBuffer().subarray(0, 170);
  assert.equal(parsePoolDepositAccount(truncatedBeforeDeferred), null);

  const truncatedTail = buildPoolDepositBuffer().subarray(0, 267);
  assert.equal(parsePoolDepositAccount(truncatedTail), null);
});

test('parsePoolStateAccount decodes current pool layout and rejects truncated pool names', () => {
  const poolState = parsePoolStateAccount(buildPoolStateBuffer());
  assert.ok(poolState);
  assert.equal(poolState?.name, 'PrimePool');
  assert.equal(poolState?.accountVersion, 1);
  assert.equal(poolState?.pendingWithdrawalRequests, 3);
  assert.equal(poolState?.status, 'Active');

  const truncatedAtPoolName = buildPoolStateBuffer().subarray(0, 60);
  assert.equal(parsePoolStateAccount(truncatedAtPoolName), null);
});
...
test('parseProposalAccount returns null when Option<u8> principal frequency is truncated', () => {
  const truncatedProposal = buildProposalBuffer().subarray(0, 90);
  assert.equal(parseProposalAccount(truncatedProposal), null);
});
...
test('all parser functions return null for invalid or corrupted buffers', () => {
  ...
  assert.equal(parsePoolStateAccount(wrongDiscriminator), null);
...
test('parser diagnostics are opt-in and strict mode throws parse errors', () => {
  const truncated = buildPoolStateBuffer().subarray(0, 60);
  const diagnostics: Array<{ parser: string; message: string }> = [];

  const parsed = parsePoolStateAccount(truncated, {
  ...
  assert.deepEqual(diagnostics, [
    {
      parser: 'PoolState',
      message: 'Pool name read out of bounds (offset=48, bufferLength=60)',
    },
  ]);
  assert.throws(
    () => parsePoolStateAccount(truncated, { strict: true }),
    /Pool name read out of bounds/
  );
});

test('parser diagnostics cover truncated reserved bytes in strict mode', () => {
  const truncatedReserved = buildEscrowBuffer().subarray(0, 200);
  const diagnostics: Array<{ parser: string; message: string }> = [];

  const parsed = parseEscrowAccount(truncatedReserved, {
    onError: (error, context) => {
      diagnostics.push({
        parser: context.parser,
        message: error instanceof Error ? error.message : String(error),
  ...
  assert.deepEqual(diagnostics, [
    {
      parser: 'LenderEscrow',
      message: 'LenderEscrow reserved bytes are truncated',
    },
  ]);
  assert.throws(
    () => parseEscrowAccount(truncatedReserved, { strict: true }),
    /LenderEscrow reserved bytes are truncated/
  );
});
