...
test('validateRevolvingRateConstraints enforces standby fee bounds for revolving contracts', () => {
  assert.doesNotThrow(() => validateRevolvingRateConstraints(true, 1200, 300));
  assert.throws(
    () => validateRevolvingRateConstraints(true, 1200, 0),
    /standbyFeeRate > 0 and < interestRate/i
  );
  assert.throws(
    () => validateRevolvingRateConstraints(true, 1200, 1200),
    /standbyFeeRate > 0 and < interestRate/i
  );
  assert.throws(
    () => validateRevolvingRateConstraints(true, 1200, 1500),
    /standbyFeeRate > 0 and < interestRate/i
  );
});

test('validateRevolvingRateConstraints allows non-revolving contracts without cross-rate checks', () => {
  assert.doesNotThrow(() => validateRevolvingRateConstraints(false, 1200, 0));
  assert.doesNotThrow(() => validateRevolvingRateConstraints(false, 1200, 1200));
  assert.doesNotThrow(() => validateRevolvingRateConstraints(false, 1200, 1500));
});
