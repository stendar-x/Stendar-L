...
import {
  ...
  DirectProcessRecallInstructionRequest,
  DirectClaimPoolYieldInstructionRequest,
  DirectSetYieldPreferenceInstructionRequest,
  DirectCompoundPoolYieldInstructionRequest,
  DirectBotClaimPoolYieldInstructionRequest,
  AnchorWalletLike,
  DirectProgramConfig,
...
interface AnchorMethodBuilder {
  accounts(accounts: Record<string, PublicKey | Keypair | null>): AnchorMethodBuilder;
  remainingAccounts(
...
function toValidatedYieldPreference(value: number): number {
  if (!Number.isInteger(value) || value < 0 || value > 1) {
    throw new Error('Invalid yieldPreference: value must be 0 (compound) or 1 (claim).');
  }
  return value;
}
...
type AnchorProviderWallet = {
  publicKey: PublicKey;
  signTransaction: <T>(transaction: T) => Promise<T>;
  signAllTransactions: <T>(transactions: T[]) => Promise<T[]>;
};

function toAnchorProviderWallet(wallet: AnchorWalletLike): AnchorProviderWallet {
  const signTransaction = wallet.signTransaction ?? (async <T>(_transaction: T): Promise<T> => {
    throw new Error('DirectProgramConfig.wallet must implement signTransaction for transaction signing.');
  });
  const signAllTransactions = wallet.signAllTransactions ?? (async <T>(_transactions: T[]): Promise<T[]> => {
    throw new Error('DirectProgramConfig.wallet must implement signAllTransactions for transaction signing.');
  });
  return {
...
export class StendarProgramClient {
  ...
  constructor(config: DirectProgramConfig) {
    ...
    const provider = new AnchorProvider(config.connection, toAnchorProviderWallet(config.wallet), {
  ...
  private async buildInstruction(
    methodName: string,
    args: unknown[],
    accounts: Record<string, PublicKey | Keypair | null>,
    remainingAccounts?: Array<{ pubkey: PublicKey; isSigner: boolean; isWritable: boolean }>
  ): Promise<TransactionInstruction> {
    const methodFactory = (
      this.program.methods as Record<string, (...methodArgs: unknown[]) => AnchorMethodBuilder>
    )[methodName];
  ...
  async createDebtContract(
    ...
    const [operationsFund] = deriveOperationsFundPda(contract, this.programId.toBase58());
    ...
    return this.buildInstruction(
      ...
      ],
      {
        ...
        frontendOperator: this.optionalPublicKey(request.frontendOperatorAddress),
        frontendUsdcAta: this.optionalPublicKey(request.frontendUsdcAta),
        tokenProgram,
  ...
  async processRecall(
    request: DirectProcessRecallInstructionRequest
  ): Promise<TransactionInstruction> {
    const treasury = request.treasuryAddress
      ? new PublicKey(request.treasuryAddress)
      : deriveTreasuryPda(this.programId.toBase58());
    ...
    return this.buildInstruction(
      ...
      [],
      {
        ...
        escrow: new PublicKey(request.escrowAddress),
        botUsdcAta: new PublicKey(request.botUsdcAta),
        contractUsdcAta: new PublicKey(request.contractUsdcAta),
        escrowUsdcAta: new PublicKey(request.escrowUsdcAta),
        treasuryUsdcAta: new PublicKey(request.treasuryUsdcAta),
        contractCollateralAta: new PublicKey(request.contractCollateralAta),
        botCollateralAta: new PublicKey(request.botCollateralAta),
  ...
  async claimPoolYield(
    request: DirectClaimPoolYieldInstructionRequest
  ): Promise<TransactionInstruction> {
    const state = request.stateAddress
      ? new PublicKey(request.stateAddress)
      : deriveGlobalStatePda(this.programId.toBase58());
    ...
    return this.buildInstruction(
      ...
      [],
      {
        depositor: new PublicKey(request.depositorAddress),
        state,
        treasury,
        pool: new PublicKey(request.poolAddress),
        poolDeposit: new PublicKey(request.poolDepositAddress),
        poolVault: new PublicKey(request.poolVaultAddress),
        depositorUsdcAta: new PublicKey(request.depositorUsdcAta),
        treasuryUsdcAccount: new PublicKey(request.treasuryUsdcAccount),
        frontendUsdcAta: this.optionalPublicKey(request.frontendUsdcAta),
        tokenProgram: this.optionalPublicKey(request.tokenProgramAddress) ?? TOKEN_PROGRAM_ID,
      }
  ...
  async setYieldPreference(
    request: DirectSetYieldPreferenceInstructionRequest
  ): Promise<TransactionInstruction> {
    return this.buildInstruction(
      'setYieldPreference',
      [toValidatedYieldPreference(request.preference)],
      {
        depositor: new PublicKey(request.depositorAddress),
        pool: new PublicKey(request.poolAddress),
        poolDeposit: new PublicKey(request.poolDepositAddress),
  ...
  async compoundPoolYield(
    request: DirectCompoundPoolYieldInstructionRequest
  ): Promise<TransactionInstruction> {
    const state = request.stateAddress
      ? new PublicKey(request.stateAddress)
      : deriveGlobalStatePda(this.programId.toBase58());
    ...
    return this.buildInstruction(
      ...
      [],
      {
        caller: new PublicKey(request.callerAddress),
        state,
        treasury,
        pool: new PublicKey(request.poolAddress),
        poolDeposit: new PublicKey(request.poolDepositAddress),
  ...
  async botClaimPoolYield(
    request: DirectBotClaimPoolYieldInstructionRequest
  ): Promise<TransactionInstruction> {
    const state = request.stateAddress
      ? new PublicKey(request.stateAddress)
      : deriveGlobalStatePda(this.programId.toBase58());
