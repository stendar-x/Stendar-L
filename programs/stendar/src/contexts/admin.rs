...
pub struct AutomatedInterestTransfer<'info> {
    ...
    #[account(
        ...
        seeds = [TREASURY_SEED],
        bump,
        constraint = treasury.bot_authority == bot_processor.key() @ StendarError::UnauthorizedBotOperation
    )]
...
pub struct RegisterFrontend<'info> {
    // Permissionless frontend registration: any wallet can register itself.
    // The PDA seed includes `operator.key()`, and `operator` must sign, so
    // one wallet cannot squat another wallet's frontend operator PDA.
    #[account(
        init,
        payer = operator,
