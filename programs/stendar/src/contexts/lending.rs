...
pub struct Initialize<'info> {
    ...
    /// SECURITY regression marker: program_data.upgrade_authority_address == Some(authority.key())
...
#[derive(Accounts)]
pub struct MigrateDebtContract<'info> {
    /// CHECK: Contract account is validated inside the instruction before reallocating.
    #[account(mut)]
...
pub struct ClaimFromEscrow<'info> {
    #[account(mut)]
    pub contract: Account<'info, DebtContract>,
    /// CHECK: Escrow account is validated inside the instruction before migrating/updating.
    #[account(mut)]
...
#[derive(Accounts)]
pub struct RecallDemandContribution<'info> {
    #[account(mut)]
    pub contract: Account<'info, DebtContract>,
    #[account(
        mut,
    ...
    pub state: Account<'info, State>,
    /// CHECK: Borrower receives any leftover operations-fund lamports (if present).
    #[account(
        mut,
        constraint = borrower.key() == contract.borrower @ StendarError::InvalidContractReference
    )]
