...
use crate::state::{
    ApprovedFunder, CollateralRegistry, ContractOperationsFund, DebtContract, FrontendOperator,
    LenderContribution, LenderEscrow, State, Treasury, APPROVED_FUNDER_SEED, OPERATIONS_FUND_SEED,
    TREASURY_SEED,
};
...
pub struct ContributeToContract<'info> {
    ...
    #[account(
        mut,
        constraint = lender_usdc_account.owner == lender.key() @ StendarError::UnauthorizedPayment,
        constraint = lender_usdc_account.mint == contract.loan_mint @ StendarError::TokenAccountMismatch
    )]
    ...
    #[account(
        ...
        constraint = contract_usdc_account.owner == contract.key() @ StendarError::TokenAccountMismatch,
        constraint = contract_usdc_account.mint == contract.loan_mint @ StendarError::TokenAccountMismatch
    )]
...
pub struct LiquidateContract<'info> {
    ...
    #[account(
        ...
        seeds = [TREASURY_SEED],
        bump,
        constraint = treasury.bot_authority == liquidator.key() @ StendarError::UnauthorizedBotOperation
    )]
    pub treasury: Option<Box<Account<'info, Treasury>>>,
    /// CHECK: Validated in instruction against expected liquidator ownership and loan mint.
    #[account(mut)]
    ...
    /// CHECK: Validated in instruction against expected liquidator ownership and collateral mint.
    #[account(mut)]
...
pub struct PartialLiquidate<'info> {
    ...
    pub state: Box<Account<'info, State>>,
    pub collateral_registry: Box<Account<'info, CollateralRegistry>>,
    ...
    pub borrower: AccountInfo<'info>,
    /// CHECK: Borrower collateral ATA used to return residual collateral when liquidation completes.
    #[account(mut)]
    pub borrower_collateral_ata: Option<Account<'info, TokenAccount>>,
    /// CHECK: Optional operations fund PDA; validated via seeds in instruction cleanup.
    #[account(
        mut,
        seeds = [OPERATIONS_FUND_SEED, contract.key().as_ref()],
        bump
...
pub struct BorrowerRepayRecall<'info> {
    ...
    #[account(
        mut,
        constraint = contract_usdc_ata.key() == contract.loan_token_account @ StendarError::TokenAccountMismatch,
        constraint = contract_usdc_ata.owner == contract.key() @ StendarError::TokenAccountMismatch,
        constraint = contract_usdc_ata.mint == contract.loan_mint @ StendarError::TokenAccountMismatch,
    )]
    pub contract_usdc_ata: Box<Account<'info, TokenAccount>>,
    #[account(
        mut,
