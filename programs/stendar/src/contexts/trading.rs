...
use crate::state::{
    ContractStatus, DebtContract, FrontendOperator, LenderContribution, LenderEscrow, State,
    TradeEvent, TradeListing, TradeOffer, Treasury, CONTRIBUTION_SEED, DEBT_CONTRACT_SEED,
    ESCROW_SEED, GLOBAL_STATE_SEED, TRADE_EVENT_SEED, TRADE_LISTING_SEED, TRADE_OFFER_SEED,
    TRADE_TRANSFER_EVENT_SEED, TREASURY_SEED,
};
...
pub struct CreateTradeListing<'info> {
    #[account(
        ...
        seeds = [TRADE_LISTING_SEED, contribution.key().as_ref(), &[nonce]],
        bump
    )]
...
pub struct CreateTradeOffer<'info> {
    #[account(
        ...
        seeds = [TRADE_OFFER_SEED, listing.key().as_ref(), buyer.key().as_ref(), &[nonce]],
        bump
    )]
...
pub struct AcceptTradeOffer<'info> {
    ...
    #[account(
        mut,
        constraint = contract.status == ContractStatus::Active @ StendarError::ContractNotActive,
        constraint = contract.key() == listing.contract @ StendarError::InvalidContractReference
    )]
    ...
    #[account(
        ...
        seeds = [TRADE_EVENT_SEED, listing.key().as_ref(), &[nonce]],
        bump
    )]
    pub trade_event: Box<Account<'info, TradeEvent>>,
    #[account(
        mut,
    ...
    #[account(
        ...
        seeds = [CONTRIBUTION_SEED, contribution.contract.as_ref(), buyer.key().as_ref()],
        bump
    )]
    ...
    #[account(
        ...
        seeds = [ESCROW_SEED, contribution.contract.as_ref(), buyer.key().as_ref()],
        bump
    )]
    ...
    pub frontend_operator: Option<Box<Account<'info, FrontendOperator>>>,
    /// Frontend USDC ATA for optional fee sharing.
    #[account(mut)]
    pub frontend_usdc_ata: Option<Account<'info, TokenAccount>>,
...
#[derive(Accounts)]
pub struct CancelOrphanedTradeOffer<'info> {
    #[account(
        mut,
        constraint = offer.buyer == buyer.key() @ StendarError::UnauthorizedCancellation,
        close = buyer
    )]
    pub offer: Account<'info, TradeOffer>,
    /// CHECK: Listing may already be closed; key is validated against the offer.
    #[account(
        constraint = listing.key() == offer.listing @ StendarError::InvalidOffer
    )]
...
pub struct TransferLenderPosition<'info> {
    #[account(
        mut,
        seeds = [DEBT_CONTRACT_SEED, contract.borrower.as_ref(), &contract.contract_seed.to_le_bytes()],
        bump,
        constraint = contract.status == ContractStatus::Active @ StendarError::ContractNotActive
    )]
    pub contract: Box<Account<'info, DebtContract>>,
    #[account(
        ...
        payer = seller,
        space = TradeEvent::LEN,
        seeds = [TRADE_TRANSFER_EVENT_SEED, contribution.key().as_ref(), &[nonce]],
        bump
    )]
...
#[derive(Accounts)]
pub struct BotCloseTradeEvent<'info> {
    #[account(
        mut,
        close = seller
    )]
    ...
    /// CHECK: Seller receives rent back when trade event closes.
    #[account(
        mut,
        constraint = seller.key() == trade_event.seller @ StendarError::InvalidContractReference
    )]
    pub seller: AccountInfo<'info>,
}
