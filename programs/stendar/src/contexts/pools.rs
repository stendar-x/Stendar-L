...
use crate::state::{
    ApprovedFunder, AuthorizedPoolOperator, DebtContract, FrontendOperator, LenderContribution,
    LenderEscrow, PendingPoolChange, PoolDeposit, PoolState, State, Treasury, APPROVED_FUNDER_SEED,
    CONTRIBUTION_SEED, ESCROW_SEED, GLOBAL_STATE_SEED, PENDING_POOL_CHANGE_SEED, POOL_DEPOSIT_SEED,
    POOL_OPERATOR_SEED, POOL_SEED, TREASURY_SEED,
};
...
#[derive(Accounts)]
pub struct CompoundPoolYield<'info> {
    ...
    pub pool_deposit: Account<'info, PoolDeposit>,
    /// CHECK: Depositor pubkey for PDA derivation. Verified by pool_deposit constraint.
    pub depositor: UncheckedAccount<'info>,
    #[account(
        mut,
        constraint = pool_vault.key() == pool.vault_token_account @ StendarError::TokenAccountMismatch
...
#[derive(Accounts)]
pub struct BotClaimPoolYield<'info> {
    ...
    pub pool_deposit: Account<'info, PoolDeposit>,
    /// CHECK: Depositor pubkey for PDA derivation and token account validation.
    pub depositor: UncheckedAccount<'info>,
    #[account(
        mut,
        constraint = pool_vault.key() == pool.vault_token_account @ StendarError::TokenAccountMismatch
...
#[derive(Accounts)]
pub struct SetYieldPreference<'info> {
    pub depositor: Signer<'info>,
    pub pool: Account<'info, PoolState>,
    #[account(
        mut,
...
pub struct PoolDeployToContract<'info> {
    ...
    #[account(
        ...
        seeds = [CONTRIBUTION_SEED, contract.key().as_ref(), pool.key().as_ref()],
        bump
    )]
    ...
    #[account(
        ...
        seeds = [ESCROW_SEED, contract.key().as_ref(), pool.key().as_ref()],
        bump
    )]
