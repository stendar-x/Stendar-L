...
pub struct PoolDeposit {
    ...
    /// Fully consumed by the frontend operator pubkey (bytes 0..32).
    /// A layout migration is required to add further fields.
    pub _reserved: [u8; 32],
    pub account_version: u16,
}
...
impl PoolDeposit {
    ...
    pub fn frontend(&self) -> Pubkey {
        Pubkey::new_from_array(self._reserved)
    }

    pub fn set_frontend(&mut self, frontend: Pubkey) {
        self._reserved.copy_from_slice(frontend.as_ref());
    }
}
