...
pub const POOL_DEPOSIT_RESERVED_BYTES: usize = ACCOUNT_RESERVED_BYTES - 8;
...
const _: [u8; ACCOUNT_RESERVED_BYTES] = [0u8; 8 + POOL_DEPOSIT_RESERVED_BYTES];
...
pub struct PoolDeposit {
    ...
    /// Yield accrued while pool liquidity could not fully back it yet.
    pub deferred_yield: u64,
    pub _reserved: [u8; POOL_DEPOSIT_RESERVED_BYTES],
    pub account_version: u16,
}
...
impl PoolDeposit {
    ...
    pub fn initialize(&mut self, depositor: Pubkey, pool: Pubkey, now: i64) {
        ...
        self.deferred_yield = 0;
        self._reserved = [0u8; POOL_DEPOSIT_RESERVED_BYTES];
        self.account_version = CURRENT_ACCOUNT_VERSION;
    }
}
...
mod tests {
    ...
    fn sample_pool_deposit() -> PoolDeposit {
        let mut deposit = PoolDeposit {
            ...
            deferred_yield: 0,
            _reserved: [0u8; POOL_DEPOSIT_RESERVED_BYTES],
            account_version: CURRENT_ACCOUNT_VERSION,
        };
    ...
    #[test]
    fn pool_deposit_deferred_yield_default_is_zero() {
        let deposit = sample_pool_deposit();
    ...
    #[test]
    fn pool_deposit_deferred_yield_roundtrip() {
        let mut deposit = sample_pool_deposit();

        deposit.deferred_yield = 1_234;

        assert_eq!(deposit.deferred_yield, 1_234);
    }
    ...
    fn pool_deposit_yield_fields_do_not_clobber_each_other() {
        ...
        deposit.deferred_yield = 777;

        assert_eq!(deposit.yield_preference, 1);
        assert_eq!(deposit.total_yield_compounded, 999);
        assert_eq!(deposit.deferred_yield, 777);
    }
}
