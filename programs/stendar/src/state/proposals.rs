...
impl TermAmendmentProposal {
    pub const DEFAULT_MAX_PARTICIPANTS: usize = 15;
    ...
    pub const BASE_LEN: usize = 8 // discriminator
        ...
        + 2; // account_version
    pub const LEN: usize = Self::space(Self::DEFAULT_MAX_PARTICIPANTS);
...
mod tests {
        ...
        assert_eq!(
            TermAmendmentProposal::space(0)
                + (TermAmendmentProposal::PARTICIPANT_KEY_BYTES
                    * TermAmendmentProposal::DEFAULT_MAX_PARTICIPANTS),
    ...
    #[test]
    fn proposal_vote_len_matches_design() {
        let sample = ProposalVote {
            proposal: Pubkey::new_unique(),
            voter: Pubkey::new_unique(),
            vote: VoteChoice::Approve,
            voted_at: 1_700_000_000,
            recall_on_rejection: false,
            _reserved: [0u8; ACCOUNT_RESERVED_BYTES],
            account_version: CURRENT_ACCOUNT_VERSION,
        };
        let serialized = sample.try_to_vec().expect("serialize proposal vote");
        assert_eq!(serialized.len() + 8, ProposalVote::LEN);
    }

    #[test]
    fn proposer_cooldown_len_matches_design() {
        let mut sample = ProposerCooldown {
        ...
        sample.initialize_defaults(Pubkey::new_unique(), Pubkey::new_unique());
        sample.cooldown_until = 1_700_086_400;

        let serialized = sample.try_to_vec().expect("serialize proposer cooldown");
        assert_eq!(serialized.len() + 8, ProposerCooldown::LEN);
    }
}
