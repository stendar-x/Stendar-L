...
pub const DEBT_CONTRACT_SEED: &[u8] = b"debt_contract";
...
impl DebtContract {
    ...
    pub fn calculate_next_interest_payment(&self) -> i64 {
        if self.last_interest_update == 0 {
            self.created_at
                .saturating_add(self.interest_frequency.to_seconds())
        } else {
            self.last_interest_update
                .saturating_add(self.interest_frequency.to_seconds())
        }
    }
    ...
    pub fn calculate_next_principal_payment(&self) -> i64 {
        if let Some(principal_freq) = self.principal_frequency {
            if self.last_principal_payment == 0 {
                ...
                self.last_principal_payment
                    .saturating_add(principal_freq.to_seconds())
            }
        } else {
            0
        }
