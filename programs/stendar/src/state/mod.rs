...
pub const LENDER_CONTRIBUTION_RESERVED_BYTES: usize = 32;
pub const LENDER_ESCROW_RESERVED_BYTES: usize = 32;
pub const APPROVED_FUNDER_RESERVED_BYTES: usize = 32;
pub const FUNDING_ACCESS_MODE_RESERVED_INDEX: usize = 0;
pub const HAS_ACTIVE_PROPOSAL_RESERVED_INDEX: usize = 1;
pub const PROPOSAL_COUNT_RESERVED_START_INDEX: usize = 2;
pub const PROPOSAL_COUNT_RESERVED_END_INDEX: usize = PROPOSAL_COUNT_RESERVED_START_INDEX + 8;
...
impl State {
    pub const LEN: usize = 8 + 32 + 8 + 8 + 8 + 8 + 8 + 2 + 1 + 2;
}
...
impl DebtContract {
    ...
    pub fn funding_access_mode(&self) -> FundingAccessMode {
        FundingAccessMode::from_reserved_byte(self._reserved[FUNDING_ACCESS_MODE_RESERVED_INDEX])
    }

    pub fn set_funding_access_mode(&mut self, mode: FundingAccessMode) {
        self._reserved[FUNDING_ACCESS_MODE_RESERVED_INDEX] = mode.to_reserved_byte();
    }

    pub fn has_active_proposal(&self) -> bool {
        self._reserved[HAS_ACTIVE_PROPOSAL_RESERVED_INDEX] == 1
    }

    pub fn set_has_active_proposal(&mut self, active: bool) {
        self._reserved[HAS_ACTIVE_PROPOSAL_RESERVED_INDEX] = if active { 1 } else { 0 };
    }

    pub fn proposal_count(&self) -> u64 {
        let mut count_bytes = [0u8; 8];
        count_bytes.copy_from_slice(
            &self._reserved[PROPOSAL_COUNT_RESERVED_START_INDEX..PROPOSAL_COUNT_RESERVED_END_INDEX],
        );
...
impl Treasury {
    pub const LEN: usize = 8 + 32 + 32 + 8 + 8 + 8 + 8 + 8 + 8 + 32 + 32 + 8 + 8 + 2;
...
mod tests {
    ...
    fn base_layout_bytes_are_unchanged_with_appended_fields() {
        ...
        assert_eq!(&full_bytes[..base_layout_bytes.len()], &base_layout_bytes[..]);
    }
    ...
    #[test]
    fn state_len_includes_is_paused_flag() {
        let expected: usize = 8 + 32 + 8 + 8 + 8 + 8 + 8 + 2 + 1 + 2;
