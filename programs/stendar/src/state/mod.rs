...
pub struct DebtContract {
    ...
    /// Contract schema marker (2 for standard contracts).
    ...
    pub term_days: u32,
    pub ltv_ratio: u32,
    /// Borrower-set minimum LTV in basis points.
    ...
    /// SPL token mint used as collateral.
    pub collateral_mint: Pubkey,
    /// ATA holding collateral for this contract.
    pub collateral_token_account: Pubkey,
    /// USDC value of collateral at creation.
    pub collateral_value_at_creation: u64,

    // === Loan Token ===
    /// USDC mint for contracts.
    ...
    // === Demand Recall ===
    /// Whether a demand recall is pending.
    pub recall_requested: bool,
    /// Recall request timestamp (0 if none).
    pub recall_requested_at: i64,
    /// Lender who requested recall.
    pub recall_requested_by: Pubkey,
...
impl DebtContract {
    ...
    pub const BASE_LEN: usize = 8  // discriminator
        ...
        + 4  // ltv_ratio
        + 4  // ltv_floor_bps
        + 1  // interest_payment_type
        + 1  // principal_payment_type
        + 1  // interest_frequency
        + 2  // principal_frequency (Option<u8>)
        + 8  // collateral_amount
        + 32 // collateral_mint
        + 32 // collateral_token_account
        + 8  // collateral_value_at_creation
        + 32 // loan_mint
        + 32 // loan_token_account
        + 8  // funded_amount
        + 2  // max_lenders
        + 1  // partial_funding_flag
        + 1  // funding_access_mode
        + 1  // allow_partial_fill
        + 2  // min_partial_fill_bps
        + 1  // status
        + 8  // created_at
        + 8  // expires_at
        + 8  // listing_fee_paid
        ...
        + 8  // accrued_interest
        + 8  // total_principal_paid
        + 8  // total_prepayment_fees
        + 8  // uncollectable_balance
        + 8  // last_interest_update
        + 8  // last_principal_payment
        + 8  // last_bot_update
        + 8  // next_interest_payment_due
        + 8  // next_principal_payment_due
        + 8  // bot_operation_count
        + 4  // num_contributions
        + 4  // contributions vec len
        + 1  // recall_requested
        + 8  // recall_requested_at
        + 32 // recall_requested_by
        + 1  // is_revolving
        + 8  // credit_limit
        + 8  // drawn_amount
        + 8  // available_amount
        + 4  // standby_fee_rate
        + 8  // accrued_standby_fees
        + 8  // last_standby_fee_update
        + 4  // total_draws
        + 8  // total_standby_fees_paid
        + 1  // revolving_closed
...
impl LenderEscrow {
    ...
    pub const LEN: usize =
        8 + 32 + 32 + 8 + 8 + 8 + 8 + 1 + 8 + 32 + ACCOUNT_RESERVED_BYTES + 2;
}
...
mod tests {
    ...
    fn lender_contribution_len_after_field_consolidation() {
        ...
        let expected: usize =
            8 + 32 + 32 + 8 + 8 + 8 + 8 + 1 + 8 + 8 + ACCOUNT_RESERVED_BYTES + 2;
    ...
    fn frontend_operator_len_matches_design() {
        assert_eq!(FrontendOperator::LEN, 8 + 32 + 8 + ACCOUNT_RESERVED_BYTES + 2);
