...
pub const ACCOUNT_RESERVED_BYTES: usize = 96;
...
pub const YIELD_PREFERENCE_COMPOUND: u8 = 0;
pub const YIELD_PREFERENCE_CLAIM: u8 = 1;
...
pub const FRONTEND_FEE_SHARE_BPS: u16 = 8_000; // 80% frontend share on protocol fees.
pub const MIN_FRONTEND_FEE_SHARE_BPS: u16 = 5_000; // Compile-time floor of 50%.
pub const MAX_FRONTEND_FEE_SHARE_BPS: u16 = 9_000; // Compile-time ceiling of 90%.
...
impl CollateralRegistry {
    ...
    pub const LEN: usize = 8
        ...
        + 1
        + 4
        + (Self::MAX_COLLATERAL_TYPES * Self::COLLATERAL_TYPE_SIZE)
...
/// Global platform state — created once during initialization.
#[account]
pub struct State {
    ...
    pub authority: Pubkey,
    pub total_debt: u64,
    /// Best-effort collateral telemetry used for platform stats.
    ///
    /// This value is not part of liquidation/health logic. It can mix value-quoted
    /// and raw-token updates across contract lifecycles, so consumers should treat
    /// it as approximate and compute precise aggregates off-chain when needed.
...
impl State {
    pub const LEN: usize =
        8 + 32 + 8 + 8 + 8 + 8 + 8 + 8 + 2 + 2 + 2 + 2 + 2 + 2 + 1 + ACCOUNT_RESERVED_BYTES + 2;
}

/// Single lending contract between borrowers and lenders.
#[account]
pub struct DebtContract {
    ...
    /// Frontend operator associated with this contract.
    pub frontend: Pubkey,
...
impl DebtContract {
    ...
    pub const DEFAULT_CONTRIBUTION_SLOTS: u16 = 14;
    ...
    pub const BASE_LEN: usize = 8
        ...
        + 32
        + ACCOUNT_RESERVED_BYTES;
    pub const LEN: usize = Self::space(Self::DEFAULT_CONTRIBUTION_SLOTS);
    ...
    pub fn calculate_next_interest_payment(&self) -> i64 {
        if self.last_interest_update == 0 {
            self.created_at.saturating_add(self.interest_frequency.to_seconds())
        } else {
            self.last_interest_update.saturating_add(self.interest_frequency.to_seconds())
        }
    }
    ...
    pub fn calculate_next_principal_payment(&self) -> i64 {
        if let Some(principal_freq) = self.principal_frequency {
            if self.last_principal_payment == 0 {
                self.created_at.saturating_add(principal_freq.to_seconds())
            } else {
                self.last_principal_payment.saturating_add(principal_freq.to_seconds())
            }
        } else {
            0
        }
...
impl LenderContribution {
    pub const LEN: usize = 8 + 32 + 32 + 8 + 8 + 8 + 8 + 1 + 8 + 8 + ACCOUNT_RESERVED_BYTES + 2;
...
impl LenderEscrow {
    ...
    pub const LEN: usize = 8 + 32 + 32 + 8 + 8 + 8 + 8 + 1 + 8 + 32 + ACCOUNT_RESERVED_BYTES + 2;
}
...
impl ApprovedFunder {
    pub const LEN: usize = 8 + 32 + 32 + 32 + 8 + ACCOUNT_RESERVED_BYTES + 2;
}
...
impl FrontendOperator {
    pub const LEN: usize = 8 + 32 + 8 + ACCOUNT_RESERVED_BYTES + 2;
}
...
pub struct Treasury {
    ...
    /// Cumulative USDC-equivalent value of liquidation fees earned by the bot.
    ///
    /// This is a telemetry counter, not a treasury token balance. The fee is
    /// implicitly captured via the collateral markup used in liquidation flows.
...
impl Treasury {
    pub const LEN: usize =
        8 + 32 + 32 + 32 + 8 + 8 + 8 + 8 + 8 + 8 + 32 + 32 + 8 + 8 + ACCOUNT_RESERVED_BYTES + 2;
...
pub struct PlatformStats {
    pub total_contracts: u64,
    pub total_debt: u64,
    /// Mirrors State.total_collateral and is approximate telemetry only.
    pub total_collateral: u64,
    pub total_interest_paid: u64,
    pub total_liquidations: u64,
    pub total_partial_liquidations: u64,
...
mod tests {
    ...
    fn max_contributions() -> Vec<Pubkey> {
        (0..usize::from(DebtContract::DEFAULT_CONTRIBUTION_SLOTS))
    ...
    fn collateral_registry_len_matches_design() {
        assert_eq!(
            CollateralRegistry::LEN,
            8 + 32 + 1 + 4 + (20 * 70) + ACCOUNT_RESERVED_BYTES + 2
        );
    }
    ...
    #[test]
    fn collateral_registry_max_capacity_serialization_matches_len() {
        let mut collateral_types = Vec::with_capacity(CollateralRegistry::MAX_COLLATERAL_TYPES);
        for _ in 0..CollateralRegistry::MAX_COLLATERAL_TYPES {
            collateral_types.push(CollateralType {
                mint: Pubkey::new_unique(),
                oracle_price_feed: Pubkey::new_unique(),
                decimals: 9,
                liquidation_buffer_bps: 600,
                min_committed_floor_bps: 5_500,
                is_active: true,
            });
        }
        ...
        let registry = CollateralRegistry {
            authority: Pubkey::new_unique(),
            num_collateral_types: CollateralRegistry::MAX_COLLATERAL_TYPES as u8,
            collateral_types,
            _reserved: [0u8; ACCOUNT_RESERVED_BYTES],
            account_version: CURRENT_ACCOUNT_VERSION,
        };
        let serialized = registry
            .try_to_vec()
            .expect("serialize max collateral registry");
        assert_eq!(serialized.len() + 8, CollateralRegistry::LEN);
    }
    ...
    fn debt_contract_len_matches_layout() {
        ...
        assert_eq!(
            DebtContract::LEN,
            DebtContract::space(DebtContract::DEFAULT_CONTRIBUTION_SLOTS)
        );
    ...
    fn lender_contribution_len_after_field_consolidation() {
        ...
        // 1 is_refunded + 8 created_at + 8 last_contributed_at + 96 _reserved + 2 account_version
        let expected: usize = 8 + 32 + 32 + 8 + 8 + 8 + 8 + 1 + 8 + 8 + ACCOUNT_RESERVED_BYTES + 2;
    ...
    fn state_len_includes_fee_fields_and_is_paused_flag() {
        ...
        let sample = State {
            ...
            total_interest_paid: 12_000,
            total_liquidations: 5,
            total_partial_liquidations: 9,
            total_contracts: 32,
            platform_fee_basis_points: 50,
            pool_deposit_fee_bps: 10,
            pool_yield_fee_bps: 12,
            primary_listing_fee_bps: 8,
            secondary_listing_fee_bps: 6,
            secondary_buyer_fee_bps: 4,
            is_paused: false,
            _reserved: [0u8; ACCOUNT_RESERVED_BYTES],
            account_version: CURRENT_ACCOUNT_VERSION,
        };
        let serialized = sample.try_to_vec().expect("serialize State");
        assert_eq!(serialized.len() + 8, State::LEN);
    }
    ...
    fn frontend_operator_len_matches_design() {
        assert_eq!(
            FrontendOperator::LEN,
            8 + 32 + 8 + ACCOUNT_RESERVED_BYTES + 2
        );
    ...
    #[test]
    fn lender_escrow_len_matches_design() {
        let expected: usize = 8 + 32 + 32 + 8 + 8 + 8 + 8 + 1 + 8 + 32 + ACCOUNT_RESERVED_BYTES + 2;
        assert_eq!(LenderEscrow::LEN, expected);

        let sample = LenderEscrow {
            lender: Pubkey::new_unique(),
            contract: Pubkey::new_unique(),
            escrow_amount: 500_000,
            available_interest: 10_000,
            available_principal: 50_000,
            total_claimed: 5_000,
            is_released: false,
            created_at: 1_700_000_000,
        ...
        let serialized = sample.try_to_vec().expect("serialize LenderEscrow");
        assert_eq!(serialized.len() + 8, LenderEscrow::LEN);
    }

    #[test]
    fn approved_funder_len_matches_design() {
        let expected: usize = 8 + 32 + 32 + 32 + 8 + ACCOUNT_RESERVED_BYTES + 2;
        assert_eq!(ApprovedFunder::LEN, expected);

        let sample = ApprovedFunder {
            contract: Pubkey::new_unique(),
            lender: Pubkey::new_unique(),
        ...
        let serialized = sample.try_to_vec().expect("serialize ApprovedFunder");
        assert_eq!(serialized.len() + 8, ApprovedFunder::LEN);
    }

    #[test]
    fn treasury_len_matches_design() {
        let expected: usize =
            8 + 32 + 32 + 32 + 8 + 8 + 8 + 8 + 8 + 8 + 32 + 32 + 8 + 8 + ACCOUNT_RESERVED_BYTES + 2;
        assert_eq!(Treasury::LEN, expected);
        ...
        let sample = Treasury {
            ...
            automated_operations: 120,
            total_contracts_processed: 33,
            last_update: 1_700_000_100,
            created_at: 1_700_000_000,
            usdc_mint: Pubkey::new_unique(),
            treasury_usdc_account: Pubkey::new_unique(),
            total_liquidation_fees: 10_000,
            total_recall_fees: 3_000,
            _reserved: [0u8; ACCOUNT_RESERVED_BYTES],
            account_version: CURRENT_ACCOUNT_VERSION,
        };
        let serialized = sample.try_to_vec().expect("serialize Treasury");
        assert_eq!(serialized.len() + 8, Treasury::LEN);
    }

    #[test]
    fn contract_operations_fund_len_matches_design() {
        let sample = ContractOperationsFund {
            contract: Pubkey::new_unique(),
            borrower: Pubkey::new_unique(),
            total_funded: 250_000,
            total_reimbursed: 100_000,
            estimated_operations: 80,
        ...
        let serialized = sample
            .try_to_vec()
            .expect("serialize ContractOperationsFund");
        assert_eq!(serialized.len() + 8, ContractOperationsFund::LEN);
    }

    #[test]
    fn debt_contract_frontend_field_roundtrip() {
        let mut contract = sample_standard_contract();
    ...
    #[test]
    fn debt_contract_frontend_default_is_pubkey_default() {
        let contract = sample_standard_contract();
    ...
    #[test]
    fn frontend_fee_share_bps_within_bounds() {
        assert!(FRONTEND_FEE_SHARE_BPS >= MIN_FRONTEND_FEE_SHARE_BPS);
        assert!(FRONTEND_FEE_SHARE_BPS <= MAX_FRONTEND_FEE_SHARE_BPS);
    }
}
...
#[event]
pub struct YieldPreferenceUpdated {
    pub pool: Pubkey,
    pub depositor: Pubkey,
    pub old_preference: u8,
    pub new_preference: u8,
    pub timestamp: i64,
}

#[event]
pub struct PoolYieldCompounded {
    pub pool: Pubkey,
    pub depositor: Pubkey,
    pub gross_yield: u64,
    pub yield_fee: u64,
    pub net_compounded: u64,
    pub new_deposit_amount: u64,
    pub timestamp: i64,
}

#[event]
pub struct PoolYieldAutoClaimed {
    pub pool: Pubkey,
    pub depositor: Pubkey,
    pub gross_yield: u64,
    pub yield_fee: u64,
