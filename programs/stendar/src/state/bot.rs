...
impl BotOperationStats {
    ...
    pub fn update_batch_stats(
        ...
        self.contracts_processed = self.contracts_processed.saturating_add(contracts_count);
        self.total_amount_processed = self.total_amount_processed.saturating_add(amount_processed);
    ...
    pub fn update_interest_stats(&mut self, payments_count: u64, amount: u64) {
        self.interest_payments_processed =
            self.interest_payments_processed.saturating_add(payments_count);
        self.total_amount_processed = self.total_amount_processed.saturating_add(amount);
    }

    /// Update principal payment statistics
    pub fn update_principal_stats(&mut self, payments_count: u64, amount: u64) {
        self.principal_payments_processed = self
            .principal_payments_processed
            .saturating_add(payments_count);
        self.total_amount_processed = self.total_amount_processed.saturating_add(amount);
    }
}
...
#[cfg(test)]
mod tests {
    use super::BotOperationStats;

    #[test]
    fn update_batch_stats_uses_saturating_add() {
        let mut stats = BotOperationStats::new();
        stats.contracts_processed = u64::MAX - 1;
        stats.total_amount_processed = u64::MAX - 1;

        stats.update_batch_stats(10, 10, 42);

        assert_eq!(stats.contracts_processed, u64::MAX);
        assert_eq!(stats.total_amount_processed, u64::MAX);
    }

    #[test]
    fn update_payment_stats_use_saturating_add() {
        let mut stats = BotOperationStats::new();
        stats.interest_payments_processed = u64::MAX - 1;
        stats.principal_payments_processed = u64::MAX - 1;
        stats.total_amount_processed = u64::MAX - 1;

        stats.update_interest_stats(10, 10);
        assert_eq!(stats.interest_payments_processed, u64::MAX);
        assert_eq!(stats.total_amount_processed, u64::MAX);

        stats.update_principal_stats(10, 10);
        assert_eq!(stats.principal_payments_processed, u64::MAX);
        assert_eq!(stats.total_amount_processed, u64::MAX);
    }
}
