...
mod tests {
    ...
    #[test]
    fn terminal_contract_states_have_no_outgoing_transitions() {
        for terminal_status in [
            ...
            assert!(terminal_status.valid_transitions().is_empty());
        }
    }

    #[test]
    fn active_lifecycle_states_do_not_downgrade_to_open() {
        for status in [ContractStatus::Active, ContractStatus::PendingRecall] {
            let transitions = status.valid_transitions();
            assert!(!transitions.contains(&ContractStatus::OpenNotFunded));
            assert!(!transitions.contains(&ContractStatus::OpenPartiallyFunded));
        }
    }
}
