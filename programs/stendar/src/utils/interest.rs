...
pub const ANNUALIZED_BPS_DENOMINATOR: u128 = SECONDS_PER_YEAR * 10_000;
...
pub fn process_automatic_interest(contract: &mut DebtContract, current_time: i64) -> Result<()> {
    if contract.is_revolving {
        ...
        let max_accrued_interest = contract
            .credit_limit
            .saturating_mul(MAX_OUTSTANDING_BALANCE_MULTIPLIER);
        if contract.accrued_interest > max_accrued_interest {
            contract.accrued_interest = max_accrued_interest;
            msg!(
                "Revolving accrued interest capped at {}x credit limit",
...
pub fn calculate_standby_fee(
    ...
    let standby_fee = (undrawn_amount as u128)
        ...
        .and_then(|value| value.checked_div(ANNUALIZED_BPS_DENOMINATOR))
        .ok_or(StendarError::ArithmeticOverflow)?;
...
pub fn checkpoint_standby_fees(contract: &mut DebtContract, current_time: i64) -> Result<()> {
    ...
    let undrawn_amount = contract
        .credit_limit
        .checked_sub(contract.drawn_amount)
        .ok_or(StendarError::InvariantViolation)?;
    let accrued = calculate_standby_fee(undrawn_amount, contract.standby_fee_rate as u64, elapsed)?;
    if accrued > 0 {
...
pub fn calculate_recall_debt_share(
    ...
    safe_u128_to_u64(
        (outstanding_balance as u128)
            .checked_mul(contribution_amount as u128)
...
mod tests {
    ...
    #[test]
    fn revolving_interest_is_capped_by_credit_limit_multiplier() {
        let mut contract = sample_contract();
        contract.is_revolving = true;
        ...
        let twenty_years_later = contract
            .last_interest_update
            .checked_add(20 * 365 * 24 * 60 * 60)
            .expect("time math should not overflow");
        process_automatic_interest(&mut contract, twenty_years_later)
            .expect("accrual should succeed");

        assert_eq!(contract.accrued_interest, 10_000_000);
    }
    ...
    #[test]
    fn calculate_interest_overflow_returns_arithmetic_overflow() {
        let err = calculate_interest(u64::MAX, u64::MAX, i64::MAX).unwrap_err();
        assert_stendar_error(err, StendarError::ArithmeticOverflow);
    }

    #[test]
    fn calculate_standby_fee_overflow_returns_arithmetic_overflow() {
        let err = calculate_standby_fee(u64::MAX, u64::MAX, i64::MAX).unwrap_err();
        assert_stendar_error(err, StendarError::ArithmeticOverflow);
    }
    ...
    #[test]
    fn checkpoint_standby_fees_rejects_drawn_amount_above_credit_limit() {
        let mut contract = sample_contract();
        contract.is_revolving = true;
        contract.revolving_closed = false;
        contract.credit_limit = 1_000_000;
        contract.drawn_amount = 1_000_001;
        contract.standby_fee_rate = 200;
        contract.last_standby_fee_update = 1;

        let err = checkpoint_standby_fees(&mut contract, 2).unwrap_err();
        assert_stendar_error(err, StendarError::InvariantViolation);
    }
    ...
    fn checkpoint_standby_fees_accrues_only_before_close() {
        ...
        checkpoint_standby_fees(&mut contract, first_checkpoint_time)
    ...
    #[test]
    fn recall_debt_share_matches_proportional_formula() {
        let debt_share = calculate_recall_debt_share(300_000, 1_000_000, 400_000)
            .expect("debt share should compute");
        assert_eq!(debt_share, 120_000);
    }

    #[test]
    fn recall_debt_share_proportional_after_partial_payment() {
        let debt_share = calculate_recall_debt_share(300_000, 1_000_000, 400_000)
            .expect("non-revolving debt share should compute");
        assert_eq!(debt_share, 120_000);
    }

    #[test]
    fn recall_debt_share_zero_when_outstanding_is_zero() {
        let debt_share = calculate_recall_debt_share(0, 1_000_000, 400_000)
            .expect("zero outstanding should compute");
        assert_eq!(debt_share, 0);
    }

    #[test]
    fn recall_debt_share_full_outstanding_equals_contribution() {
        let debt_share = calculate_recall_debt_share(1_000_000, 1_000_000, 400_000)
            .expect("full outstanding should compute");
        assert_eq!(debt_share, 400_000);
    }

    #[test]
    fn revolving_recall_split_keeps_total_principal_conserved() {
        let recall_amount = 400_000u64;
        let debt_share = calculate_recall_debt_share(300_000, 1_000_000, recall_amount)
            .expect("debt share should compute");
        let borrower_repay = debt_share;
        let pool_repay = recall_amount
            .checked_sub(borrower_repay)
            .expect("split math should not underflow");

        assert_eq!(borrower_repay, 120_000);
        assert_eq!(pool_repay, 280_000);
        assert_eq!(borrower_repay + pool_repay, recall_amount);
    }
}
