...
#[allow(dead_code)]
pub fn calculate_platform_fee_usdc(amount: u64) -> Result<u64> {
...
#[allow(dead_code)]
pub fn calculate_secondary_market_fee_usdc(amount: u64) -> Result<u64> {
    calculate_fee_with_bps(
        amount,
...
mod tests {
    ...
    fn platform_fee_is_pct_or_floor() {
        ...
        assert_eq!(calculate_platform_fee(1).unwrap(), MIN_PLATFORM_FEE);
        assert_eq!(calculate_platform_fee(9_999).unwrap(), MIN_PLATFORM_FEE);

        // 1 SOL = 1_000_000_000 lamports => 1bp = 100_000 lamports, still below the floor.
        assert_eq!(calculate_platform_fee(1_000_000_000).unwrap(), MIN_PLATFORM_FEE);

        // 100 SOL => 100_000_000_000 lamports => 1bp = 10_000_000 lamports > floor.
        assert_eq!(calculate_platform_fee(100_000_000_000).unwrap(), 10_000_000);
    }
    ...
    fn secondary_market_fee_is_10bps_or_floor() {
        ...
        assert_eq!(calculate_secondary_market_fee(9_999).unwrap(), MIN_PLATFORM_FEE);
    ...
    fn secondary_market_fee_usdc_uses_usdc_floor() {
        assert_eq!(
            calculate_secondary_market_fee_usdc(1).unwrap(),
            MIN_PLATFORM_FEE_USDC
        );
        assert_eq!(
            calculate_secondary_market_fee_usdc(9_999).unwrap(),
            MIN_PLATFORM_FEE_USDC
        );

        // 100 USDC => 100_000_000 atomic units => 10bp = 100_000.
        assert_eq!(
            calculate_secondary_market_fee_usdc(100_000_000).unwrap(),
            100_000
        );
    }
    ...
    fn secondary_market_fee_usdc_is_capped_at_max() {
        ...
        assert_eq!(
            calculate_secondary_market_fee_usdc(20_000_000_000).unwrap(),
            MAX_PLATFORM_FEE_USDC
        );
    }
    ...
    fn usdc_platform_fee_uses_usdc_floor() {
        ...
        assert_eq!(calculate_platform_fee_usdc(1).unwrap(), MIN_PLATFORM_FEE_USDC);
        assert_eq!(
            calculate_platform_fee_usdc(9_999).unwrap(),
            MIN_PLATFORM_FEE_USDC
        );

        // 1,000 USDC in 6-decimal units => 1_000_000_000 atomic units => 1bp = 100_000.
        assert_eq!(calculate_platform_fee_usdc(1_000_000_000).unwrap(), 100_000);
    }
    ...
    fn usdc_platform_fee_is_capped_at_max() {
        ...
        // so the cap should apply.
        assert_eq!(
            calculate_platform_fee_usdc(20_000_000_000).unwrap(),
            MAX_PLATFORM_FEE_USDC
        );
    }
