...
mod tests {
    ...
    #[test]
    fn ceil_div_u16_rounds_up_without_saturation() {
        assert_eq!(ceil_div_u16(14, 14).unwrap(), 1);
        assert_eq!(ceil_div_u16(15, 14).unwrap(), 2);
        assert_eq!(ceil_div_u16(u16::MAX, u16::MAX).unwrap(), 1);
        assert_eq!(ceil_div_u16(u16::MAX, u16::MAX - 1).unwrap(), 2);
    }
    ...
    fn frontend_share_normal() {
        assert_eq!(calculate_frontend_share(10_000).unwrap(), 8_000);
    }

    #[test]
    fn frontend_share_odd() {
        assert_eq!(calculate_frontend_share(3).unwrap(), 2);
    }
