...
pub fn calculate_position_value(
    ...
    let days_remaining = (time_remaining / (24 * 60 * 60)).max(0) as u32;
...
mod tests {
    use super::*;
    use crate::state::{
        InterestPaymentType, PaymentFrequency, PrincipalPaymentType,
        CURRENT_ACCOUNT_VERSION, DEBT_CONTRACT_RESERVED_BYTES,
        LENDER_CONTRIBUTION_RESERVED_BYTES,
    };
    ...
    #[test]
    fn position_valuation_expired_contract() {
        ...
        let result = calculate_position_value(&contribution, &contract, current_time);
        assert!(result.is_err(), "expired contract should not be tradeable");
    }
