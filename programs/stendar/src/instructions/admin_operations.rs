...
use crate::state::{
    AuthorityUpdated, ContractStatus, FeeRatesUpdated, FrontendRegistered, InterestPaymentType,
    LenderContribution, LenderEscrow, PlatformPauseToggled, PlatformStats, TreasuryWithdrawal,
    ACCOUNT_RESERVED_BYTES, CURRENT_ACCOUNT_VERSION, TREASURY_SEED,
};
use crate::utils::{
...
pub fn initialize_treasury(
    ...
    treasury._reserved = [0u8; ACCOUNT_RESERVED_BYTES];
...
pub fn force_reset_platform(ctx: Context<ForceResetPlatform>) -> Result<()> {
    let state_info = &ctx.accounts.state;
    let data = state_info.try_borrow_data()?;
    require!(data.len() >= 40, StendarError::InvalidAuthority);
    let stored_authority = Pubkey::try_from(&data[8..40]).map_err(|_| StendarError::InvalidAuthority)?;
    drop(data);
    require!(
    ...
    fn close_pda(pda: &AccountInfo, dest: &AccountInfo) {
        let lamports = pda.lamports();
        **dest.try_borrow_mut_lamports().unwrap() += lamports;
        **pda.try_borrow_mut_lamports().unwrap() = 0;
        pda.try_borrow_mut_data().unwrap().fill(0);
        pda.assign(&anchor_lang::system_program::ID);
        pda.realloc(0, false).ok();
    }

    close_pda(state_info, &authority_info);
    close_pda(&ctx.accounts.treasury, &authority_info);

    msg!("Platform accounts force-closed for migration reset");
    Ok(())
}
...
pub fn register_frontend(ctx: Context<RegisterFrontend>) -> Result<()> {
    let frontend_operator = &mut ctx.accounts.frontend_operator;
    let clock = Clock::get()?;

    frontend_operator.operator = ctx.accounts.operator.key();
    frontend_operator.registered_at = clock.unix_timestamp;
    frontend_operator._reserved = [0u8; ACCOUNT_RESERVED_BYTES];
    frontend_operator.account_version = CURRENT_ACCOUNT_VERSION;

    emit!(FrontendRegistered {
        operator: ctx.accounts.operator.key(),
        frontend_operator_pda: frontend_operator.key(),
        timestamp: clock.unix_timestamp,
    });

    msg!(
        "Frontend registered: operator={}",
...
mod tests {
    ...
    #[test]
    fn register_frontend_pda_derivation() {
        let operator = Pubkey::new_unique();
        let (frontend_operator_pda, frontend_operator_bump) = Pubkey::find_program_address(
            &[crate::state::FRONTEND_OPERATOR_SEED, operator.as_ref()],
            &crate::ID,
        );

        let derived_with_bump = Pubkey::create_program_address(
            &[
                crate::state::FRONTEND_OPERATOR_SEED,
                operator.as_ref(),
                &[frontend_operator_bump],
            ],
            &crate::ID,
        )
        .expect("frontend operator PDA should derive with bump");

        assert_eq!(frontend_operator_pda, derived_with_bump);
    }
}
