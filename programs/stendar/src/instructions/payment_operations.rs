...
pub fn make_payment_with_distribution<'info>(
    ...
    let result = (|| -> Result<()> {
        ...
        for chunk in remaining_accounts.chunks(chunk_size) {
            ...
            let lender_interest = if interest_payment > 0 {
                safe_u128_to_u64(
                    (contribution.contribution_amount as u128)
                        .checked_mul(interest_payment as u128)
            ...
            let lender_principal = if principal_payment > 0 {
                safe_u128_to_u64(
                    (contribution.contribution_amount as u128)
                        .checked_mul(principal_payment as u128)
                        .and_then(|v| v.checked_div(total_funded as u128))
                        .ok_or(StendarError::ArithmeticOverflow)?,
                )?
            } else {
