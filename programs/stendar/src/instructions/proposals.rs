...
use crate::utils::{
    calculate_frontend_share, calculate_proportional_collateral, calculate_recall_debt_share,
...
fn proposal_has_pending_recalls_for_close(
    status: ProposalStatus,
    recall_pledged_count: u32,
    recalls_processed: u32,
) -> bool {
    matches!(status, ProposalStatus::Rejected | ProposalStatus::Expired)
        && recall_pledged_count > 0
        && recalls_processed < recall_pledged_count
}
...
pub fn process_proposal_recall(
    ...
    let debt_share = calculate_recall_debt_share(
        ...
        ctx.accounts.contract.funded_amount,
        recall_amount,
    )?;
    let recall_fee = calculate_recall_fee(debt_share)?;
    let lender_receives = debt_share
...
pub fn close_proposal_accounts(
    ...
    let proposal = &ctx.accounts.proposal;
    require!(!proposal.is_pending(), StendarError::ProposalNotPending);
    let has_pending_recalls = proposal_has_pending_recalls_for_close(
        proposal.status,
        proposal.recall_pledged_count,
        proposal.recalls_processed,
    );
    require!(!has_pending_recalls, StendarError::ProposalRecallsPending);
...
mod tests {
    ...
    #[test]
    fn close_guard_blocks_rejected_or_expired_with_unprocessed_pledges() {
        assert!(proposal_has_pending_recalls_for_close(
    ...
    #[test]
    fn close_guard_allows_non_recall_statuses() {
        assert!(!proposal_has_pending_recalls_for_close(
    ...
    #[test]
    fn close_guard_allows_when_no_pending_recall_work_remains() {
        assert!(!proposal_has_pending_recalls_for_close(
            ProposalStatus::Rejected,
            0,
            0
