...
use crate::state::{
    ContractFunded, ContractStatus, LoanType, PoolChangeApplied, PoolChangeCancelled,
    PoolChangeProposed, PoolDeposit, PoolExpired, PoolState, PoolStatus, CURRENT_ACCOUNT_VERSION,
    POOL_CHANGE_TIMELOCK_SECONDS, POOL_IDLE_EXPIRY_SECONDS, POOL_SEED,
};
use crate::utils::{calculate_fee_tenths_bps, require_current_version};
...
pub fn create_pool(
    ...
    pool.is_variable_rate = is_variable_rate;
...
pub fn update_pool_rate(ctx: Context<UpdatePoolRate>, new_rate_bps: u32) -> Result<()> {
    require_current_version(ctx.accounts.pool.account_version)?;
    require_pool_active(&ctx.accounts.pool)?;
    require!(new_rate_bps <= 10_000, StendarError::InvalidPaymentAmount);
    let proposed_at = Clock::get()?.unix_timestamp;
    require_rate_update_interval(ctx.accounts.pool.rate_updated_at, proposed_at)?;
    ...
    pending_change.rate_bps = Some(new_rate_bps);
    pending_change.capacity = None;
    pending_change.minimum_deposit = None;
    pending_change.allowed_loan_type = None;
    pending_change.min_ltv_bps = None;
    pending_change.max_term_days = None;
    pending_change.withdrawal_queue_enabled = None;
