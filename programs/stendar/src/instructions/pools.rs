...
fn resolve_rate_at_timestamp(pool: &PoolState, timestamp: i64) -> u32 {
    ...
    if history_count == 0 {
        // Backward compatibility path for pools created before the history buffer existed.
        if pool.rate_updated_at > 0 && pool.prev_rate_bps > 0 {
            return pool.prev_rate_bps;
        }
        return pool.rate_bps;
    }
