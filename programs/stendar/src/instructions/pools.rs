...
use crate::state::{
    ...
    PoolYieldAutoClaimed, PoolYieldClaimed, PoolYieldCompounded, RateHistoryEntry,
    YieldPreferenceUpdated, ACCOUNT_RESERVED_BYTES, CURRENT_ACCOUNT_VERSION,
    POOL_CHANGE_TIMELOCK_SECONDS, POOL_IDLE_EXPIRY_SECONDS, POOL_SEED, YIELD_PREFERENCE_CLAIM,
    YIELD_PREFERENCE_COMPOUND,
};
use crate::utils::{
    calculate_fee_tenths_bps, calculate_frontend_share, require_current_version, BPS_DENOMINATOR,
};
...
fn current_rate_start(pool: &PoolState) -> i64 {
    if pool.rate_updated_at > 0 {
        pool.rate_updated_at
    } else {
        pool.created_at
    }
}

fn resolve_rate_at_timestamp(pool: &PoolState, timestamp: i64) -> u32 {
    let current_start = current_rate_start(pool);
    if timestamp >= current_start {
        return pool.rate_bps;
    }

    let history_count = pool.rate_history_entry_count();
    if history_count == 0 {
        return pool.rate_bps;
    }

    let mut resolved_rate = pool
        .rate_history_entry_at(0)
        .map(|entry| entry.rate_bps)
        .unwrap_or(pool.rate_bps);
    for idx in 0..history_count {
        let Some(entry) = pool.rate_history_entry_at(idx) else {
            break;
        };
        if entry.changed_at <= timestamp {
            resolved_rate = entry.rate_bps;
            continue;
        }
        break;
    }
...
/// Accrues depositor yield using piecewise segments across the pool's on-chain
...
fn accrue_deposit_yield(
    ...
    let mut newly_accrued = 0u64;
    let mut segment_start = pool_deposit.last_yield_update;
    let mut active_rate_bps = resolve_rate_at_timestamp(pool, segment_start);

    for idx in 0..pool.rate_history_entry_count() {
        let Some(entry) = pool.rate_history_entry_at(idx) else {
            break;
        };
        if entry.changed_at <= segment_start {
            continue;
        }
        ...
        let elapsed = entry
            ...
            .ok_or(StendarError::ArithmeticOverflow)?;
        let segment_yield =
            calculate_accrued_yield(pool_deposit.deposit_amount, active_rate_bps, elapsed)?;
        newly_accrued = newly_accrued
            .checked_add(segment_yield)
            .ok_or(StendarError::ArithmeticOverflow)?;
        segment_start = entry.changed_at;
        active_rate_bps = entry.rate_bps;
    }

    let current_start = current_rate_start(pool);
    if current_start > segment_start && current_start < now {
        let elapsed = current_start
            .checked_sub(segment_start)
            .ok_or(StendarError::ArithmeticOverflow)?;
        let segment_yield =
            calculate_accrued_yield(pool_deposit.deposit_amount, active_rate_bps, elapsed)?;
        ...
        active_rate_bps = pool.rate_bps;
    }

    let elapsed_current = now
        .checked_sub(segment_start)
        .ok_or(StendarError::ArithmeticOverflow)?;
    let accrued_current = calculate_accrued_yield(
    ...
    let credited = total_owed.min(remaining_yield_capacity);
    ...
    if credited > 0 {
        pool_deposit.accrued_yield = pool_deposit
            ...
            .ok_or(StendarError::ArithmeticOverflow)?;
        pool.add_pending_yield(credited)?;
    }

    pool_deposit.deferred_yield = total_owed
...
fn transfer_yield_fee<'info>(
    yield_fee: u64,
    stored_frontend: Pubkey,
    loan_mint: Pubkey,
    pool_vault: &Account<'info, TokenAccount>,
    treasury_usdc_account: &Account<'info, TokenAccount>,
    frontend_usdc_ata: Option<&Account<'info, TokenAccount>>,
    ...
    if frontend_share > 0 {
        let frontend_ata = frontend_usdc_ata.ok_or(StendarError::MissingTokenAccounts)?;
...
pub fn claim_pool_yield(ctx: Context<ClaimPoolYield>) -> Result<()> {
    ...
    if yield_fee > 0 {
        let treasury_fee_received = transfer_yield_fee(
            yield_fee,
            pool_deposit.frontend,
            pool.loan_mint,
            &ctx.accounts.pool_vault,
            &ctx.accounts.treasury_usdc_account,
            ctx.accounts.frontend_usdc_ata.as_ref(),
    ...
    emit!(PoolYieldClaimed {
        pool: pool_key,
        depositor: depositor_key,
        gross_yield: yield_amount,
        yield_fee,
        net_claimed: net_yield,
        timestamp: now,
    });
...
pub fn set_yield_preference(ctx: Context<SetYieldPreference>, preference: u8) -> Result<()> {
    require!(
        preference == YIELD_PREFERENCE_COMPOUND || preference == YIELD_PREFERENCE_CLAIM,
        StendarError::InvalidYieldPreference
    );

    let pool_deposit = &mut ctx.accounts.pool_deposit;
    require_current_version(pool_deposit.account_version)?;

    let old = pool_deposit.yield_preference;
    pool_deposit.yield_preference = preference;
...
/// Compounds accrued depositor yield into principal.
/// Intentionally does not require an active pool so yield can be managed during pause.
pub fn compound_pool_yield(ctx: Context<CompoundPoolYield>) -> Result<()> {
    let now = Clock::get()?.unix_timestamp;
    let depositor_key = ctx.accounts.depositor.key();
    ...
    let is_depositor = caller_key == depositor_key;
    let is_bot = caller_key == ctx.accounts.treasury.bot_authority;
    require!(
        is_depositor || is_bot,
        StendarError::UnauthorizedBotOperation
    );
    if is_bot && !is_depositor {
        require!(
            pool_deposit.yield_preference == YIELD_PREFERENCE_COMPOUND,
            StendarError::YieldPreferenceMismatch
        );
    }
    ...
    // Only protocol fees leave the vault; net yield remains and is reclassified as principal.
    require!(
        ctx.accounts.pool_vault.amount >= yield_fee,
        StendarError::InsufficientPoolLiquidity
    );
    ...
    pool_deposit.deposit_amount = pool_deposit
        ...
        .ok_or(StendarError::ArithmeticOverflow)?;
    pool_deposit.accrued_yield = 0;
    pool_deposit.total_yield_compounded = pool_deposit
        .total_yield_compounded
        .checked_add(net_yield)
        .ok_or(StendarError::ArithmeticOverflow)?;

    // Compounding reclassifies in-vault yield into principal and may exceed nominal capacity.
    pool.current_total_deposits = pool
        .current_total_deposits
        .checked_add(net_yield)
        .ok_or(StendarError::ArithmeticOverflow)?;
    ...
    emit!(PoolYieldCompounded {
        pool: pool_key,
        depositor: depositor_key,
        gross_yield: yield_amount,
        yield_fee,
        net_compounded: net_yield,
        new_deposit_amount: pool_deposit.deposit_amount,
        timestamp: now,
    });
...
/// Bot-only auto-claim path for depositors who opted into claim preference.
/// Intentionally does not require an active pool so yield remains claimable during pause.
pub fn bot_claim_pool_yield(ctx: Context<BotClaimPoolYield>) -> Result<()> {
    let now = Clock::get()?.unix_timestamp;
    let depositor_key = ctx.accounts.depositor.key();
    require_current_version(ctx.accounts.state.account_version)?;
    require_current_version(ctx.accounts.treasury.account_version)?;
    ...
    require!(
        pool_deposit.yield_preference == YIELD_PREFERENCE_CLAIM,
        StendarError::YieldPreferenceMismatch
    );
...
pub fn apply_pool_changes(ctx: Context<ApplyPoolChanges>) -> Result<()> {
    ...
    if let Some(rate) = rate_bps {
        let outgoing_rate_start = if pool.rate_updated_at > 0 {
            pool.rate_updated_at
        } else {
            pool.created_at
        };
        let outgoing_rate_bps = pool.rate_bps;
        pool.push_rate_history(RateHistoryEntry {
            changed_at: outgoing_rate_start,
            rate_bps: outgoing_rate_bps,
        });
        pool.prev_rate_bps = outgoing_rate_bps;
        pool.rate_bps = rate;
...
mod tests {
    ...
    fn sample_pool() -> PoolState {
        PoolState {
            ...
            rate_history_len: 0,
            rate_history_head: 0,
            rate_history: [crate::state::RateHistoryEntry::default();
                crate::state::RATE_HISTORY_SIZE],
            _reserved: [0u8; crate::state::RATE_HISTORY_PADDING_BYTES],
    ...
    fn sample_deposit(last_yield_update: i64) -> PoolDeposit {
        PoolDeposit {
            ...
            deferred_yield: 0,
            _reserved: [0u8; crate::state::POOL_DEPOSIT_RESERVED_BYTES],
            account_version: CURRENT_ACCOUNT_VERSION,
        }
    }

    #[test]
    fn accrue_deposit_yield_uses_current_rate_when_rate_timestamp_has_no_history_entries() {
        let mut pool = sample_pool();
        // Regression coverage: `rate_updated_at` is set, but no historical entries were persisted.
    ...
    fn accrue_deposit_yield_tracks_pending_yield() {
        ...
        assert_eq!(deposit.deferred_yield, 0);
    }

    #[test]
    fn accrue_deposit_yield_walks_rate_history_for_multiple_changes() {
        let mut pool = sample_pool();
        pool.initialize_rate_history();
        pool.push_rate_history(RateHistoryEntry {
            changed_at: 0,
        ...
        pool.prev_rate_bps = 750;
        pool.rate_bps = 500;
        pool.rate_updated_at = 300;
        let mut deposit = sample_deposit(50);
        pool.current_total_deposits = deposit.deposit_amount;

        let accrued =
            accrue_deposit_yield(&mut pool, &mut deposit, 350).expect("accrual must succeed");
        let expected = calculate_accrued_yield(deposit.deposit_amount, 1_000, 50).unwrap()
            + calculate_accrued_yield(deposit.deposit_amount, 1_500, 100).unwrap()
            + calculate_accrued_yield(deposit.deposit_amount, 750, 100).unwrap()
            + calculate_accrued_yield(deposit.deposit_amount, 500, 50).unwrap();
    ...
    #[test]
    fn accrue_deposit_yield_handles_rate_history_ring_buffer_wrap() {
        let mut pool = sample_pool();
        pool.initialize_rate_history();
        for index in 0..8u32 {
            pool.push_rate_history(RateHistoryEntry {
                changed_at: (index as i64) * 100,
                rate_bps: 1_000 + (index * 100),
            });
        }
        pool.prev_rate_bps = 1_700;
        pool.rate_bps = 1_800;
        pool.rate_updated_at = 800;
        let mut deposit = sample_deposit(50);
        pool.current_total_deposits = deposit.deposit_amount;

        let accrued =
            accrue_deposit_yield(&mut pool, &mut deposit, 900).expect("accrual must succeed");
        let expected = calculate_accrued_yield(deposit.deposit_amount, 1_100, 50).unwrap()
            + calculate_accrued_yield(deposit.deposit_amount, 1_100, 100).unwrap()
            + calculate_accrued_yield(deposit.deposit_amount, 1_200, 100).unwrap()
            + calculate_accrued_yield(deposit.deposit_amount, 1_300, 100).unwrap()
            + calculate_accrued_yield(deposit.deposit_amount, 1_400, 100).unwrap()
            + calculate_accrued_yield(deposit.deposit_amount, 1_500, 100).unwrap()
            + calculate_accrued_yield(deposit.deposit_amount, 1_600, 100).unwrap()
            + calculate_accrued_yield(deposit.deposit_amount, 1_700, 100).unwrap()
            + calculate_accrued_yield(deposit.deposit_amount, 1_800, 100).unwrap();

        assert_eq!(
            pool.rate_history_entry_count(),
            crate::state::RATE_HISTORY_SIZE
        );
        assert_eq!(accrued, expected);
    }
    ...
    #[test]
    fn accrue_deposit_yield_defers_when_pool_has_no_backing() {
        let mut pool = sample_pool();
        pool.current_total_deposits = 0;
        pool.current_utilized = 0;
        ...
        let expected_newly =
            calculate_accrued_yield(deposit.deposit_amount, pool.rate_bps, now).unwrap();

        let credited = accrue_deposit_yield(&mut pool, &mut deposit, now)
            .expect("accrual should defer when no liquidity backs yield");

        assert_eq!(credited, 0);
        assert_eq!(deposit.accrued_yield, 0);
        assert_eq!(pool.total_pending_yield(), 0);
        assert_eq!(deposit.deferred_yield, expected_newly);
        assert_eq!(deposit.last_yield_update, now);
    }

    #[test]
    fn accrue_deposit_yield_partially_credits_and_defers() {
        let mut pool = sample_pool();
        ...
        assert_eq!(deposit.deferred_yield, expected_newly - 10);
        assert_eq!(deposit.last_yield_update, now);
    }

    #[test]
    fn accrue_deposit_yield_credits_deferred_when_capacity_returns() {
        let mut pool = sample_pool();
        let mut deposit = sample_deposit(0);
        pool.current_total_deposits = 0;
        pool.current_utilized = 0;

        let first_checkpoint = SECONDS_PER_YEAR as i64;
        let second_checkpoint = first_checkpoint * 2;
        let first_newly =
            calculate_accrued_yield(deposit.deposit_amount, pool.rate_bps, first_checkpoint)
                .unwrap();
        let second_newly =
            calculate_accrued_yield(deposit.deposit_amount, pool.rate_bps, first_checkpoint)
                .unwrap();

        let first_credited = accrue_deposit_yield(&mut pool, &mut deposit, first_checkpoint)
            .expect("first accrual should defer when no backing exists");
        assert_eq!(first_credited, 0);
        assert_eq!(deposit.deferred_yield, first_newly);

        pool.current_total_deposits = first_newly;
        pool.current_utilized = 0;
        let second_credited = accrue_deposit_yield(&mut pool, &mut deposit, second_checkpoint)
            .expect("returned capacity should credit deferred yield");

        assert_eq!(second_credited, first_newly);
        assert_eq!(deposit.accrued_yield, first_newly);
        assert_eq!(pool.total_pending_yield(), first_newly);
        assert_eq!(deposit.deferred_yield, second_newly);
        assert_eq!(deposit.last_yield_update, second_checkpoint);
    }
    ...
    fn clear_withdrawal_request_updates_pool_counter() {
        ...
        clear_withdrawal_request(&mut pool, &mut deposit).expect("clear should succeed");
    ...
    fn clear_withdrawal_request_is_idempotent() {
        ...
        clear_withdrawal_request(&mut pool, &mut deposit).expect("first clear should succeed");
        clear_withdrawal_request(&mut pool, &mut deposit).expect("second clear should succeed");
        assert_eq!(pool.pending_withdrawal_requests(), 0);
        assert!(!deposit.withdrawal_requested);
        assert_eq!(deposit.withdrawal_requested_at, 0);
        assert_eq!(deposit.withdrawal_requested_amount, 0);
