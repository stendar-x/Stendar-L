...
use crate::utils::{
    ...
    check_revolving_completion, checkpoint_standby_fees, get_price_in_usdc,
    process_automatic_interest, require_current_version, safe_u128_to_u64,
    MAX_CONFIDENCE_BPS_STANDARD, MAX_PRICE_AGE_DRAW,
};
...
fn can_sweep_contract_pool(status: ContractStatus) -> bool {
    matches!(
        status,
...
/// Validates that every contribution+escrow pair passed via `remaining_accounts` matches the
/// on-chain PDAs stored in the contract. Cost is O(n) in the number of contributors due to
/// per-account deserialization and PDA derivation. With `max_lenders` capped at 100 by the
/// protocol, worst-case CU cost stays well within Solana's per-instruction compute budget.
...
pub fn draw_from_revolving<'info>(
    ...
    // Defense-in-depth: recall sets `revolving_closed = true`, which independently blocks draws.
    require!(
    ...
    require!(
        amount <= ctx.accounts.contract_usdc_account.amount,
        StendarError::InsufficientContractBalance
    );
    ...
    if ctx.accounts.contract.ltv_ratio > 0 {
        ...
        let (price, exponent) =
            get_price_in_usdc(price_feed, MAX_PRICE_AGE_DRAW, MAX_CONFIDENCE_BPS_STANDARD)?;
        ...
        let full_debt = ctx
            ...
            .drawn_amount
            .checked_add(amount)
            .and_then(|value| value.checked_add(ctx.accounts.contract.accrued_interest))
            .and_then(|value| value.checked_add(ctx.accounts.contract.accrued_standby_fees))
            .ok_or(StendarError::ArithmeticOverflow)?;
        let current_ltv = calculate_ltv_bps(collateral_value, full_debt)?;
        require!(
    ...
    ctx.accounts.state.total_debt = ctx
        .accounts
        .state
        .total_debt
        .checked_add(amount)
...
/// Closes a revolving facility.
///
/// For committed facilities with remaining term (`term_days > 0`, before maturity),
/// an early termination fee is charged against the undrawn availability. That fee is
/// accrued into `accrued_standby_fees`, so the contract does not transition to
/// `Completed` until a later `distribute_standby_fees` call clears the accrued fee
/// balance.
pub fn close_revolving_facility(ctx: Context<CloseRevolvingFacility>) -> Result<()> {
    ...
    if contract.loan_type == LoanType::Committed
        && contract.term_days > 0
        && available_before_close > 0
    {
...
pub fn bot_close_matured_revolving(ctx: Context<BotCloseMaturedRevolving>) -> Result<()> {
    ...
    require!(
        ctx.accounts.contract.loan_type == LoanType::Committed,
        StendarError::CannotCancelContract
    );
    require!(
        ctx.accounts.contract.term_days > 0,
        StendarError::CannotCancelContract
    );
    ...
    let maturity = contract
        ...
        .checked_add((contract.term_days as i64).saturating_mul(24 * 60 * 60))
        .ok_or(StendarError::ArithmeticOverflow)?;
    require!(current_time >= maturity, StendarError::ContractNotExpired);
...
pub fn sweep_contract_pool(ctx: Context<SweepContractPool>) -> Result<()> {
    ...
    require_revolving_contract(&ctx.accounts.contract)?;
    require!(
        can_sweep_contract_pool(ctx.accounts.contract.status),
        StendarError::CannotCancelContract
    );
    require!(
        ctx.accounts.contract.contributions.is_empty(),
        StendarError::InvalidContribution
    );
    require!(
        ctx.accounts.contract_usdc_account.owner == ctx.accounts.contract.key()
    ...
    let sweep_amount = ctx.accounts.contract_usdc_account.amount;
    if sweep_amount == 0 {
        return Ok(());
    }
...
pub fn distribute_standby_fees<'info>(
    ...
    require!(
        ctx.accounts.contract_usdc_account.amount >= standby_fee_amount,
        StendarError::InsufficientContractBalance
    );
    ...
    // Standby fees are transferred out of the same USDC pool that backs future draws.
    // Keep `credit_limit` as the contractual ceiling, but reduce `available_amount`
    // so borrowers cannot draw liquidity that has already been paid to lenders.
    if !contract.revolving_closed {
        contract.available_amount = contract
            .available_amount
...
#[cfg(test)]
mod tests {
    use super::can_sweep_contract_pool;
    use crate::state::ContractStatus;

    #[test]
    fn sweep_is_allowed_for_terminal_statuses() {
        assert!(can_sweep_contract_pool(ContractStatus::Completed));
        assert!(can_sweep_contract_pool(ContractStatus::Liquidated));
    }

    #[test]
    fn sweep_is_rejected_for_non_terminal_statuses() {
        assert!(!can_sweep_contract_pool(ContractStatus::OpenNotFunded));
        assert!(!can_sweep_contract_pool(
            ContractStatus::OpenPartiallyFunded
        ));
        assert!(!can_sweep_contract_pool(ContractStatus::Active));
        assert!(!can_sweep_contract_pool(ContractStatus::PendingRecall));
        assert!(!can_sweep_contract_pool(ContractStatus::Cancelled));
    }
}
