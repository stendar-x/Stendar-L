...
use crate::state::{
    ContractStatus, DebtContract, FrontendFeeSplit, LenderContribution, LenderEscrow, ListingType,
    TradeEvent, TradeListing, TradeType, Treasury, ACCOUNT_RESERVED_BYTES, CURRENT_ACCOUNT_VERSION,
    MIN_LISTING_AMOUNT,
};
use crate::utils::{
    calculate_fee_tenths_bps, calculate_frontend_share, require_current_version, safe_u128_to_u64,
    validate_trade_conditions, MAX_LENDERS_PER_TX,
};
...
pub fn create_trade_listing(
    ...
    listing.nonce = nonce;
    listing._reserved = [0u8; ACCOUNT_RESERVED_BYTES];
    listing.account_version = CURRENT_ACCOUNT_VERSION;
...
pub fn create_trade_offer(
    ...
    let listing = &mut ctx.accounts.listing;
    require_current_version(listing.account_version)?;
    let current_time = Clock::get()?.unix_timestamp;
    ...
    offer.nonce = nonce;
    offer._reserved = [0u8; ACCOUNT_RESERVED_BYTES];
    offer.account_version = CURRENT_ACCOUNT_VERSION;
...
pub fn accept_trade_offer(ctx: Context<AcceptTradeOffer>, nonce: u8) -> Result<()> {
    ...
    let offer = &mut ctx.accounts.offer;
    require_current_version(listing.account_version)?;
    require_current_version(offer.account_version)?;
    let current_time = Clock::get()?.unix_timestamp;
    ...
    let payment_accounts = TradePaymentAccounts {
        ...
        treasury_usdc: ctx
            ...
            .as_ref()
            .map(|a| a.clone()),
        frontend_usdc: ctx.accounts.frontend_usdc_ata.as_ref().map(|a| a.clone()),
...
pub fn bot_close_trade_event(ctx: Context<BotCloseTradeEvent>) -> Result<()> {
    require_current_version(ctx.accounts.treasury.account_version)?;
    require_current_version(ctx.accounts.state.account_version)?;
    require_current_version(ctx.accounts.trade_event.account_version)?;
    require!(
...
pub fn execute_position_transfer(
    ...
    buyer_contribution.last_contributed_at = current_time;
    buyer_contribution._reserved = [0u8; ACCOUNT_RESERVED_BYTES];
    ...
    buyer_escrow.escrow_token_account = Pubkey::default();
    buyer_escrow._reserved = [0u8; ACCOUNT_RESERVED_BYTES];
    ...
    trade_event._reserved = [0u8; ACCOUNT_RESERVED_BYTES];
    trade_event.account_version = CURRENT_ACCOUNT_VERSION;
...
pub struct TradePaymentAccounts<'info> {
    ...
    pub frontend_usdc: Option<Account<'info, TokenAccount>>,
...
pub fn handle_payment_transfer<'info>(
    ...
    let mut treasury_fee_received = total_protocol_fee;
    if total_protocol_fee > 0 {
        ...
        let frontend_share = if let Some(frontend_pubkey) = frontend {
            if let Some(frontend_ata) = accounts.frontend_usdc.as_ref() {
                require!(
                    frontend_ata.owner == frontend_pubkey,
                    StendarError::FrontendTokenAccountMismatch
                );
                require!(
                    frontend_ata.mint == treasury_usdc.mint,
                    StendarError::TokenAccountMismatch
                );
                calculate_frontend_share(total_protocol_fee)?
            } else {
                0
            }
        } else {
        ...
        if frontend_share > 0 {
            let frontend_pubkey = frontend.ok_or(StendarError::ArithmeticOverflow)?;
            let frontend_ata = accounts
                .frontend_usdc
                .as_ref()
                .ok_or(StendarError::MissingTokenAccounts)?;
            let treasury_share = total_protocol_fee
            ...
            emit!(FrontendFeeSplit {
                frontend: frontend_pubkey,
                fee_type: 3,
                total_fee: total_protocol_fee,
                frontend_share,
                treasury_share,
            });
        } else {
...
mod tests {
    ...
    use crate::state::{
        ...
        PrincipalPaymentType, ACCOUNT_RESERVED_BYTES,
    };
