...
use crate::state::{
    ...
    CURRENT_ACCOUNT_VERSION, DEMAND_LOAN_MIN_FLOOR_BPS, LENDER_CONTRIBUTION_RESERVED_BYTES,
    LENDER_ESCROW_RESERVED_BYTES, LIQUIDATION_FEE_BPS, MAX_PROTOCOL_LENDERS, OPERATIONS_FUND_SEED,
    PARTIAL_LIQUIDATION_CAP_BPS, RECALL_FEE_BPS, RECALL_GRACE_PERIOD_SECONDS, RESERVED_TAIL_BYTES,
};
use crate::utils::{
    ...
    calculate_recall_debt_share, check_health, check_revolving_completion, checkpoint_standby_fees,
...
pub fn initialize_state(ctx: Context<Initialize>) -> Result<()> {
    ...
    // Pool/trade fee fields use tenths-of-bps precision (1 = 0.001%).
    state.pool_deposit_fee_bps = 1;
    state.pool_yield_fee_bps = 1;
    state.primary_listing_fee_bps = 1;
    state.secondary_listing_fee_bps = 1;
    state.secondary_buyer_fee_bps = 1;
...
fn finalize_full_liquidation(
    ...
    if contract.is_revolving {
        debug_assert_eq!(
            ...
            "revolving outstanding_balance must equal drawn_amount at liquidation",
        );
    }
    ...
    state.total_collateral = state
        .total_collateral
        .checked_sub(pre_liquidation_collateral)
        .ok_or(StendarError::ArithmeticOverflow)?;
...
pub fn borrower_repay_recall(ctx: Context<BorrowerRepayRecall>) -> Result<()> {
    ...
    let debt_share = calculate_recall_debt_share(
        ...
        funded_amount,
        repay_amount,
        ctx.accounts.contract.is_revolving,
    )?;
    let borrower_repay_amount = if ctx.accounts.contract.is_revolving {
        debt_share
    } else {
        repay_amount
    };
    let pool_repay_amount = repay_amount
...
mod tests {
    ...
    fn sample_state() -> State {
        State {
            ...
            pool_deposit_fee_bps: 1,
            pool_yield_fee_bps: 1,
            primary_listing_fee_bps: 1,
            secondary_listing_fee_bps: 1,
            secondary_buyer_fee_bps: 1,
            is_paused: false,
            account_version: CURRENT_ACCOUNT_VERSION,
        }
    }
    ...
    fn initialize_lender_contribution_fields_sets_cooldown_timestamp() {
        ...
        let mut contribution = LenderContribution {
            ...
            _reserved: [1u8; LENDER_CONTRIBUTION_RESERVED_BYTES],
            account_version: 0,
        };
