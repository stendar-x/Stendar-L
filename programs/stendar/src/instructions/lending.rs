...
use crate::state::{
    ...
    ContractStatus, ContributionWithdrawn, DebtContract, DistributionMethod, FrontendFeeSplit,
    FundingAccessMode, InterestPaymentType, LenderContribution, LenderEscrow, LoanType,
    PaymentFrequency, PrincipalPaymentType, State, ACCOUNT_RESERVED_BYTES, CURRENT_ACCOUNT_VERSION,
...
fn liquidate_contract_standard<'info>(
    ...
    // Liquidation intentionally bypasses platform pause checks to protect lenders.
    let contract_key = ctx.accounts.contract.key();
    let borrower_info = ctx.accounts.borrower.to_account_info();
    ...
    let (
        ...
        require!(
            contract.status == ContractStatus::Active
                || contract.status == ContractStatus::PendingRecall,
            StendarError::ContractNotEligibleForLiquidation
        );
        (
    ...
    emit!(ContractLiquidated {
        contract: contract_key,
        bot_pays_usdc,
        collateral_seized: capped_collateral_for_bot,
        shortfall: ctx.accounts.contract.uncollectable_balance,
        liquidation_fee: liquidation_fee_usdc,
        is_partial: false,
    });
...
pub fn partial_liquidate<'info>(
    ...
    let contract = &ctx.accounts.contract;
    let contract_borrower = contract.borrower;
    let collateral_mint = contract.collateral_mint;
    ...
    let debt_for_ltv = calculate_liquidation_debt(contract)?;
    ...
    let max_repay = std::cmp::min(max_repay, contract.outstanding_balance);
    ...
    let current_time = Clock::get()?.unix_timestamp;
    let (completed_by_partial_liquidation, completion_shortfall) = {
        let contract = &mut ctx.accounts.contract;
        ...
        if completed {
            ...
            shortfall = contract.accrued_interest;
            contract.uncollectable_balance = contract
                .uncollectable_balance
    ...
    if completed_by_partial_liquidation {
        let borrower_info = ctx.accounts.borrower.to_account_info();
        let residual_collateral = ctx
            .accounts
            .contract_collateral_ata
            .amount
            .saturating_sub(actual_seize);
        let mut transferred_residual = false;
        if residual_collateral > 0 {
            if let Some(borrower_collateral_ata) = ctx.accounts.borrower_collateral_ata.as_ref() {
                require!(
                    borrower_collateral_ata.owner == contract_borrower
                        && borrower_collateral_ata.mint == collateral_mint,
                    StendarError::TokenAccountMismatch
                ...
                transferred_residual = true;
            } else {
                msg!(
                    "partial_liquidate completion missing borrower_collateral_ata; residual collateral remains in contract ATA"
                );
            }
        }

        if residual_collateral == 0 || transferred_residual {
            token::close_account(CpiContext::new_with_signer(
                ctx.accounts.token_program.to_account_info(),
                CloseAccount {
                    account: ctx.accounts.contract_collateral_ata.to_account_info(),
                    destination: borrower_info.clone(),
                    authority: ctx.accounts.contract.to_account_info(),
                },
                signer_seeds,
            ...
            msg!(
                "partial_liquidate completion skipped collateral ATA close because residual collateral was not transferred"
            );
        }
    ...
    emit!(ContractLiquidated {
        contract: contract_key,
        bot_pays_usdc: capped_repay,
        collateral_seized: actual_seize,
        shortfall: completion_shortfall,
        liquidation_fee: liquidation_fee_usdc,
        is_partial: true,
    });
...
mod tests {
    ...
    #[test]
    fn partial_liquidation_health_debt_uses_total_non_revolving_debt() {
        let mut contract = sample_contract(113);
        contract.is_revolving = false;
        contract.interest_payment_type = InterestPaymentType::CollateralTransfer;
        contract.outstanding_balance = 900_000;
        contract.accrued_interest = 120_000;
        ...
        let debt_for_ltv =
            calculate_liquidation_debt(&contract).expect("health debt should include accrued");
        assert_eq!(debt_for_ltv, 1_020_000);
    }

    #[test]
    fn partial_liquidation_repay_cap_clamps_to_outstanding_balance() {
        let mut contract = sample_contract(114);
        contract.is_revolving = false;
        contract.interest_payment_type = InterestPaymentType::CollateralTransfer;
        contract.outstanding_balance = 200_000;
        contract.accrued_interest = 600_000;

        let debt_for_ltv =
            calculate_liquidation_debt(&contract).expect("debt for ltv should calculate");
        assert_eq!(debt_for_ltv, 800_000);
        ...
        let max_repay = safe_u128_to_u64(
            (debt_for_ltv as u128)
                ...
                .and_then(|value| value.checked_div(10_000))
                .ok_or(StendarError::ArithmeticOverflow)
                .expect("max repay calculation should not overflow"),
        )
        .expect("max repay should fit in u64");
        let max_repay = std::cmp::min(max_repay, contract.outstanding_balance);

        assert_eq!(max_repay, 200_000);
    }

    #[test]
    fn partial_liquidation_completion_tracks_shortfall_and_resets_collateral() {
        let mut contract = sample_contract(115);
        contract.status = ContractStatus::Active;
        contract.is_revolving = false;
        contract.outstanding_balance = 200_000;
        contract.accrued_interest = 75_000;
        contract.collateral_amount = 300_000;
        contract.total_principal_paid = 0;
        contract.uncollectable_balance = 0;

        let capped_repay = 200_000u64;
        let actual_seize = 50_000u64;
        let current_time = 1_703_000_000;

        contract.outstanding_balance = contract
            .outstanding_balance
            .checked_sub(capped_repay)
            .expect("repay should settle outstanding");
        contract.collateral_amount = contract
            .collateral_amount
            .checked_sub(actual_seize)
            .expect("seize should reduce collateral");
        contract.total_principal_paid = contract
            .total_principal_paid
            .checked_add(capped_repay)
            .expect("principal paid should increase");

        let completed = contract.outstanding_balance == 0;
        assert!(completed);
        if completed {
            let shortfall = contract.accrued_interest;
            contract.uncollectable_balance = contract
                .uncollectable_balance
                .checked_add(shortfall)
                .expect("shortfall tracking should not overflow");
            contract.accrued_interest = 0;
            contract.collateral_amount = 0;
            contract.status = ContractStatus::Completed;
        }
        ...
        assert_eq!(contract.uncollectable_balance, 75_000);
        assert_eq!(contract.accrued_interest, 0);
        assert_eq!(contract.collateral_amount, 0);
    }

    #[test]
    fn contract_liquidated_event_roundtrip_includes_new_fields() {
        let event = ContractLiquidated {
            contract: Pubkey::new_unique(),
            bot_pays_usdc: 123_000,
            collateral_seized: 456_000,
            shortfall: 7_000,
            liquidation_fee: 3_690,
            is_partial: true,
        };
        let encoded = event.try_to_vec().expect("event should serialize");
        let decoded = ContractLiquidated::try_from_slice(&encoded).expect("event should deserialize");

        assert_eq!(decoded.contract, event.contract);
        assert_eq!(decoded.bot_pays_usdc, event.bot_pays_usdc);
        assert_eq!(decoded.collateral_seized, event.collateral_seized);
        assert_eq!(decoded.shortfall, event.shortfall);
        assert_eq!(decoded.liquidation_fee, event.liquidation_fee);
        assert_eq!(decoded.is_partial, event.is_partial);
    }
