...
use crate::state::{
    CollateralRegistry, CollateralType, ACCOUNT_RESERVED_BYTES, CURRENT_ACCOUNT_VERSION,
};
use crate::utils::{require_current_version, validate_price_feed_registration_account};
...
pub fn initialize_collateral_registry(ctx: Context<InitializeCollateralRegistry>) -> Result<()> {
    ...
    registry._reserved = [0u8; ACCOUNT_RESERVED_BYTES];
    registry.account_version = CURRENT_ACCOUNT_VERSION;
    Ok(())
}
...
pub fn add_collateral_type(
    ...
    let registry = &mut ctx.accounts.collateral_registry;
    require_current_version(registry.account_version)?;
