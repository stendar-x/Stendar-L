...
compile_error!(
    "The `testing` feature must not be enabled for mainnet builds."
);
...
pub mod stendar {
    ...
    /// Deprecated: use `propose_pool_changes` + `apply_pool_changes` instead.
    pub fn update_pool_rate(ctx: Context<UpdatePoolRate>, new_rate_bps: u32) -> Result<()> {
        instructions::update_pool_rate(ctx, new_rate_bps)
    }
