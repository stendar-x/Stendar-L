...
pub enum StendarError {
    ...
    #[msg("Internal invariant violated — program state is inconsistent")]
    InvariantViolation,
    #[msg("Contract is not eligible for liquidation in its current status")]
    ContractNotEligibleForLiquidation,
    #[msg("Proposal has pending pledged recalls that must be processed before closure")]
    ProposalRecallsPending,
    #[msg("Pool withdrawal queue is disabled")]
