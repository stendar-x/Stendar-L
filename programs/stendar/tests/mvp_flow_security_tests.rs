...
#[test]
fn mvp_secondary_market_rejects_listing_when_contract_is_not_active() {
    let contract_key = Pubkey::new_unique();
    let contribution = mock_contribution(contract_key, 1_000_000_000);
    ...
    for status in [
        ...
        assert!(result.is_err(), "status {:?} must be rejected", status);
        assert!(result
            .expect_err("inactive status should fail listing validation")
            .to_string()
            .contains("Contract is not active for trading"));
    }
}

#[test]
fn mvp_secondary_market_rejects_offer_when_listing_is_inactive_or_expired() {
    let listing_amount = 1_000_000_000;
    let seller = Pubkey::new_unique();
    let current_time = 1_700_000_100;

    let mut boundary_expired_listing = mock_listing(
        Pubkey::new_unique(),
        Pubkey::new_unique(),
        seller,
        listing_amount,
    );
    boundary_expired_listing.expires_at = current_time;
    let boundary_expired_result = validate_offer_parameters(
        &boundary_expired_listing,
        listing_amount,
        1_100_000_000,
        current_time + 3_600,
        current_time,
    );
    assert!(boundary_expired_result.is_err());
    assert!(boundary_expired_result
        .expect_err("listing expiring at current time must be invalid")
        .to_string()
        .contains("Trade listing has expired"));

    let mut inactive_listing = mock_listing(
        Pubkey::new_unique(),
        Pubkey::new_unique(),
        seller,
        listing_amount,
    ...
    let inactive_result = validate_offer_parameters(
        &inactive_listing,
        listing_amount,
        1_100_000_000,
        current_time + 3_600,
    ...
    assert!(inactive_result
        .expect_err("inactive listing must be rejected")
        .to_string()
        .contains("Trade listing has expired"));

    let mut zero_expiry_listing = mock_listing(
        Pubkey::new_unique(),
        Pubkey::new_unique(),
        seller,
        listing_amount,
    );
    zero_expiry_listing.expires_at = 0;
    let zero_expiry_result = validate_offer_parameters(
        &zero_expiry_listing,
        listing_amount,
        1_100_000_000,
        current_time + 3_600,
    ...
    assert!(zero_expiry_result.is_err());
    assert!(zero_expiry_result
        .expect_err("non-positive listing expiry must be rejected")
        .to_string()
        .contains("Trade listing has expired"));
}

#[test]
fn mvp_secondary_market_allows_offer_at_max_duration_boundary() {
    let listing_amount = 1_000_000_000;
    let seller = Pubkey::new_unique();
    let listing = mock_listing(
        Pubkey::new_unique(),
    ...
    let result = validate_offer_parameters(
        ...
        listing_amount,
        1_100_000_000,
        current_time + MAX_TRADE_LISTING_DURATION_SECONDS,
        current_time,
    );
...
#[test]
fn mvp_secondary_market_rejects_offer_with_immediate_expiry() {
    let listing_amount = 1_000_000_000;
    let seller = Pubkey::new_unique();
    let listing = mock_listing(
        Pubkey::new_unique(),
    ...
    assert!(result.is_err());
    assert!(result
        .expect_err("offer expiring at current time should be rejected")
        .to_string()
        .contains("Trade offer has expired"));
}
