...
#[test]
fn test_interest_due_status_boundaries() {
    let mut contract =
        create_mock_contract(PaymentFrequency::Weekly, Some(PaymentFrequency::Monthly));
    let current_time = 1_700_000_000;
    contract.next_interest_payment_due = current_time;

    for active_status in [ContractStatus::Active, ContractStatus::PendingRecall] {
        contract.status = active_status;
        assert!(
            contract.is_interest_payment_due(current_time),
            "status {:?} should be considered active for interest due checks",
            active_status
        );
    }
    ...
    for inactive_status in [
        ...
        contract.status = inactive_status;
        assert!(
            !contract.is_interest_payment_due(current_time),
            "status {:?} must not be considered interest-due eligible",
            inactive_status
        );
    }
}
...
#[test]
fn test_principal_due_requires_frequency_and_active_status() {
    let mut contract = create_mock_contract(PaymentFrequency::Weekly, None);
    let current_time = 1_700_000_000;
    contract.status = ContractStatus::Active;
    contract.next_principal_payment_due = current_time;
    assert!(!contract.is_principal_payment_due(current_time));
...
#[test]
fn test_update_bot_tracking_saturates_count_and_uses_creation_when_last_updates_are_zero() {
    let mut contract =
        create_mock_contract(PaymentFrequency::Weekly, Some(PaymentFrequency::Monthly));
    let current_time = 1_700_000_000;
    contract.last_interest_update = 0;
    contract.last_principal_payment = 0;
    contract.bot_operation_count = u64::MAX;

    contract.update_bot_tracking(current_time);

    assert_eq!(contract.last_bot_update, current_time);
    assert_eq!(contract.bot_operation_count, u64::MAX);
    assert_eq!(
        contract.next_interest_payment_due,
        contract.created_at + PaymentFrequency::Weekly.to_seconds()
    );
    assert_eq!(
        contract.next_principal_payment_due,
        contract.created_at + PaymentFrequency::Monthly.to_seconds()
    );
}

#[test]
fn test_update_bot_tracking_due_dates_saturate_at_i64_max() {
    let mut contract =
        create_mock_contract(PaymentFrequency::Monthly, Some(PaymentFrequency::Monthly));
    contract.last_interest_update = i64::MAX - 5;
    contract.last_principal_payment = i64::MAX - 5;

    contract.update_bot_tracking(1_700_000_000);

    assert_eq!(contract.next_interest_payment_due, i64::MAX);
    assert_eq!(contract.next_principal_payment_due, i64::MAX);
}
