...
fn meets_partial_fill_threshold(contract: &DebtContract) -> bool {
    ...
    let required_scaled = (contract.target_amount as u128)
        .saturating_mul(contract.min_partial_fill_bps as u128);
    funded_scaled >= required_scaled
}

fn simulate_primary_listing_expiration(contract: &mut DebtContract, current_time: i64) -> ExpirationOutcome {
