...
describe("Treasury Automated Operations", () => {
  ...
  async function setupTestEnvironment() {
    // Initialize platform and treasury
    ...
    // Fund contract with two lenders
    ...
    // Make payment to generate funds
  ...
  describe("Interest & Principal Transfers", () => {
    it("Processes automated interest distribution", async () => {
      ...
      // Create remaining accounts array for lenders
  ...
  describe("Performance & Efficiency", () => {
    it("Handles batch operations efficiently", async () => {
      // This test verifies the batch processing capability
      const treasury = await program.account.treasury.fetch(treasuryPda);
      const operationsBefore = treasury.automatedOperations.toNumber();

      // The automated operations process multiple lenders in single transactions
      // This is more efficient than individual transfers
      expect(operationsBefore).to.be.greaterThan(0);
      
      // Verify that transaction costs are tracked
      expect(treasury.totalTxCosts.toNumber()).to.be.greaterThan(0);
    });
  });
}); 